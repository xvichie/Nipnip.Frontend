  ...
  function buildHero(cfg: Required<ThemeConfig>) {
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {cfg.heroEyebrow && (
          <p className={`${HERO_EYEBROW_SIZE_CLASS[cfg.heroEyebrowSize]} font-black uppercase tracking-[0.3em] ${getHeroTextColorClass(cfg, 'eyebrow', 'text-white/70')} mb-4`}>{cfg.heroEyebrow}</p>
        )}
        <h1 className={`font-black uppercase tracking-tight mb-6 leading-none ${getHeroTextColorClass(cfg, 'headline', 'text-white')} ${HERO_HEADLINE_SIZE_CLASS[cfg.heroHeadlineSize]}`}>
          {cfg.heroHeadline || store.name}
        </h1>
        {cfg.heroSubheadline && (
          <p className={`${getHeroTextColorClass(cfg, 'subheadline', 'text-white/80')} ${HERO_SUBHEADLINE_SIZE_CLASS[cfg.heroSubheadlineSize]} mb-10`}>{cfg.heroSubheadline}</p>
        )}
          ...
          <div className={`flex flex-wrap items-center gap-3 ${getHeroButtonRowClass(cfg)}`}>
            ...
            {cfg.heroSecondaryCtaEnabled && (
              <Link
                ...
                className="inline-flex items-center gap-2 border-2 border-white/40 text-white text-sm font-black uppercase tracking-wide px-9 py-4 transition-colors hover:bg-white/10"
              >
    ...
    const heroImage = (
      <div className={`relative h-full ${imageOrderClass} ${heroImageHiddenMobile ? 'hidden md:block' : ''}`}>
        ...
        <div
          className={`relative h-full flex items-center justify-center overflow-hidden ${imageBanner || sectionBanner ? '' : 'bg-white/[0.04]'}`}
    ...
    return (
      <section className="relative overflow-hidden border-b border-white/10">
        {isBackgroundHero ? (
          ...
          <>
            <div className="absolute inset-0" style={sectionBanner ? bannerStyle : { backgroundColor: '#0d0f0d' }} />
            {showCenterOverlay && <div className="absolute inset-0 bg-[#0d0f0d]/60" />}
        ...
        <div className={`grid ${LANDING_CATEGORY_GRID_CLASS[tokens.landingCategoryColumns]} gap-3`}>
          {landingCategories.map(category => (
            <Link
              ...
              className="flex flex-col items-center justify-center gap-2.5 border border-white/10 bg-white/[0.04] px-4 py-6 text-center hover:border-white/25 transition-colors"
            >
              {(category.iconUrl || category.iconKey || category.iconEmoji) && (
                <span className="w-10 h-10 rounded-full flex items-center justify-center text-[#0d0f0d]" style={{ backgroundColor: tokens.accentColor }}>
                  <CategoryIcon iconUrl={category.iconUrl} iconKey={category.iconKey} iconEmoji={category.iconEmoji} className="w-5 h-5" />
                </span>
              )}
              <span className="text-sm font-bold uppercase tracking-wide text-white/80">{getCategoryName(category, lang)}</span>
            </Link>
          ))}
        </div>
      </div>
    ...
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16">
        <div className="flex items-end justify-between mb-8">
          <h2 className="font-black text-3xl text-white tracking-tight uppercase">{t.home.allProductsHeading}</h2>
          <Link
            href={`/products`}
            className="text-sm font-black uppercase tracking-wide transition-opacity hover:opacity-80"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16 flex flex-col gap-16">
        {landingCollections.map(collection => (
          <ProductScrollRow
            ...
            titleClassName="font-black text-3xl text-white tracking-tight uppercase"
            viewAllClassName="text-sm font-black uppercase tracking-wide transition-opacity hover:opacity-80"
  ...
  const contentSection = hasContentBlock(tokens) && (
    <section key="content" className="border-b border-white/10" style={getSectionBackgroundStyle(tokens, 'content')}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16">
        <ContentBlock
          tokens={tokens}
          headingClassName="font-black text-2xl sm:text-3xl text-white tracking-tight uppercase mb-4"
          bodyClassName="text-white/70 text-sm sm:text-base leading-relaxed mb-6"
          buttonClassName="inline-flex items-center gap-2 text-[#0d0f0d] text-sm font-black uppercase tracking-wide px-8 py-3.5 transition-transform hover:scale-105"
          buttonStyle={{ backgroundColor: tokens.accentColor, boxShadow: glowShadow(tokens.accentColor, '88') }}
        />
      </div>
    </section>
  )
    ...
    <section key="faq" className="border-b border-white/10" style={getSectionBackgroundStyle(tokens, 'faq')}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-16">
        <FaqAccordion
          tokens={tokens}
          t={t}
          headingClassName="font-black text-2xl sm:text-3xl text-white tracking-tight uppercase mb-6"
          questionClassName="font-bold text-sm text-white uppercase"
