...
export function ProductGrid({
  ...
  return (
    <div className="bg-[#0d0f0d] min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 pb-24">
        <div className="mb-10">
          <p className="text-xs font-black uppercase tracking-[0.2em] mb-2" style={{ color: tokens.accentColor }}>{t.grid.collectionEyebrow}</p>
          <h1 className="font-black text-4xl text-white tracking-tight uppercase">{activeCategoryName ?? activeCollectionName ?? t.grid.allProductsTitle}</h1>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-10">
          ...
          <aside className="w-full md:w-52 shrink-0 flex flex-col gap-8 md:sticky md:top-16 md:self-start md:max-h-[calc(100vh-5rem)] md:overflow-y-auto md:pb-6">
            {displayCategories.length > 0 && (
              <div className="flex flex-col gap-2">
                <p className="text-xs font-black uppercase tracking-widest text-white/40 mb-2">{t.header.categories}</p>
                <Link
                  href={`/products`}
                  className={[
                    'px-4 py-2 text-sm font-bold uppercase tracking-wide transition-colors text-left',
                    !activeCategorySlug ? 'text-[#0d0f0d]' : 'text-white/50 hover:text-white',
                  ].join(' ')}
                ...
                {displayCategories.map(category => (
                  <Link
                    ...
                    className={[
                      ...
                      activeCategorySlug === category.slug ? 'text-[#0d0f0d]' : 'text-white/50 hover:text-white',
          ...
          <div className="flex-1">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
              <p className="text-sm font-semibold text-white/50 flex items-center gap-2">
                <span className="text-white font-black">{totalCount}</span> {t.grid.unitProduct}
                {isRefetching && (
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                {tokens.searchBarLocation !== 'header' && (
                  <input
                    ...
                    className="w-full sm:w-56 bg-white/[0.06] border border-white/10 text-sm px-4 py-2 text-white placeholder:text-white/30 focus:outline-none transition-colors"
                  />
                )}
                <select
                  value={sortBy}
                  onChange={e => setSortBy(e.target.value as ProductSortOption)}
                  className="w-full sm:w-auto shrink-0 bg-white/[0.06] border border-white/10 text-sm px-4 py-2 text-white focus:outline-none transition-colors"
                >
                  {getSortOptions(t).map(opt => (
                    <option key={opt.value} value={opt.value} className="bg-[#0d0f0d]">{opt.label}</option>
                  ))}
                </select>
              </div>
            </div>
            ...
            {isLoading ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-5">
                {Array.from({ length: 8 }).map((_, i) => (
                  <div key={i} className="skeleton aspect-square bg-white/[0.04]" />
                ))}
              </div>
            ) : products.length === 0 ? (
              <div className="py-32 flex flex-col items-center gap-4 text-center">
                <p className="text-white/40 text-sm">{t.grid.noProductsFound}</p>
              </div>
            ) : (
              <>
                ...
                <Pagination
                  ...
                  onChange={setPage}
                  t={t}
                  buttonClassName="btn btn-sm bg-white/[0.06] border border-white/10 text-white/60 hover:text-white disabled:opacity-30"
