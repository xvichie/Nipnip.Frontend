      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 xl:gap-20">
          ...
          <div className="flex gap-3">
              ...
              <div className="flex flex-col gap-2 shrink-0">
                {images.map((image, i) => (
                  <button
                    ...
                    className="w-16 h-16 overflow-hidden border-2 transition-colors bg-white/[0.06]"
            ...
            <div
              className="flex-1 aspect-square bg-white/[0.06] overflow-hidden"
          ...
          <div className="flex flex-col">
            ...
            {tokens.sizeGuideContent && (
              <button
                ...
                className="text-xs font-bold uppercase tracking-wide text-white/40 hover:text-white underline underline-offset-2 mb-4 text-left w-fit"
              >
                საზომი ცხრილი
              </button>
            )}
            ...
            <div className="flex items-center gap-3 mb-4">
              <div className="flex items-center bg-white/[0.06] border border-white/10">
                <button
                  type="button"
                ...
                <span className="w-10 text-center text-white text-sm font-black tabular-nums">{quantity}</span>
                <button
                  type="button"
              ...
              {selectionComplete && stock !== null && (
                <span className={`text-xs font-bold uppercase tracking-wide ${isLowStock ? 'text-red-400' : 'text-white/40'}`}>
            ...
            <button
              ...
              className="w-full py-4 text-sm font-black uppercase tracking-wide transition-transform hover:scale-[1.02] mb-8 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100 text-[#0d0f0d]"
              style={{ backgroundColor: added ? '#2d6a2d' : tokens.accentColor, boxShadow: added ? 'none' : glowShadow(tokens.accentColor, '88'), color: added ? '#fff' : '#0d0f0d' }}
            >
              {added
                ? '✓ დაემატა კალათაში'
                : !selectionComplete
            ...
            {tokens.deliveryEstimateText && (
              <p className="text-xs text-white/40 mb-4">{tokens.deliveryEstimateText}</p>
            )}

            {tokens.trustBadges.length > 0 && (
              <div className="flex flex-wrap gap-x-4 gap-y-1 mb-8">
                {tokens.trustBadges.map((badge, i) => (
                  <span key={i} className="text-xs font-semibold text-white/40">✓ {badge}</span>
                ))}
              </div>
            )}
            ...
            {product.description && (
              <div className="pt-8 border-t border-white/10">
                <p className="text-xs font-black uppercase tracking-widest text-white/40 mb-3">აღწერა</p>
            ...
            {product.videoUrl && (
              <div className="pt-8 border-t border-white/10">
                <p className="text-xs font-black uppercase tracking-widest text-white/40 mb-3">ვიდეო</p>
        ...
        {tokens.showRelatedProducts && product.relatedProducts.length > 0 && (
          <div className="mt-16 pt-10 border-t border-white/10">
            <h2 className="font-black text-2xl text-white tracking-tight uppercase mb-6">{tokens.relatedProductsHeading || 'მსგავსი პროდუქტები'}</h2>
