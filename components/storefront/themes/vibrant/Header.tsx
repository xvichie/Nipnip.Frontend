  ...
  return (
    <header className="sticky top-0 z-50 bg-white shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-6">
        <Link href={`/store/${slug}`} className="flex items-center gap-2.5 shrink-0">
          {tokens.logoUrl ? (
            <>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={tokens.logoUrl} alt={storeName} className="h-8 w-auto object-contain" />
              {tokens.showStoreName && (
                <span className="font-black text-[#1a1a1a] text-lg tracking-tight">{storeName}</span>
              )}
            </>
          ) : (
            <span className="font-black text-[#1a1a1a] text-lg tracking-tight">{storeName}</span>
                    ...
                    <div className="absolute top-full left-0 mt-2 min-w-[190px] rounded-2xl bg-white shadow-xl border border-[#f0e4da] p-2 flex flex-col gap-0.5 z-10">
                      {navCategories.map(category => (
                        <Link
                          ...
                          className="rounded-xl px-3.5 py-2 text-sm font-medium text-[#1a1a1a]/70 hover:text-[#1a1a1a] hover:bg-[#fff2ec] transition-colors whitespace-nowrap"
                        >
                          {category.name}
                        </Link>
                      ))}
