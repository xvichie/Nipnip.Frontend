  ...
  function buildHero(cfg: Required<ThemeConfig>) {
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        ...
        <h1 className={`font-black tracking-tight mb-5 leading-[1.05] ${getHeroTextColorClass(cfg, 'headline', 'text-[#1a1a1a]')} ${HERO_HEADLINE_SIZE_CLASS[cfg.heroHeadlineSize]}`}>
          {heroHeadline || store.name}
        </h1>
        {heroSubheadline && (
          <p className={`${getHeroTextColorClass(cfg, 'subheadline', 'text-[#6b6058]')} ${HERO_SUBHEADLINE_SIZE_CLASS[cfg.heroSubheadlineSize]} mb-8`}>{heroSubheadline}</p>
        )}
          ...
          <div className={`flex flex-wrap items-center gap-3 ${getHeroButtonRowClass(cfg)}`}>
            {cfg.heroCtaEnabled && (
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-full text-sm font-bold px-8 py-4 theme-cta-btn"
                style={{ backgroundColor: cfg.accentColor, color: cfg.buttonTextColor, boxShadow: `0 16px 40px -12px ${cfg.accentColor}88`, '--btn-hover-bg': getButtonHoverColor(cfg) } as React.CSSProperties}
              >
            ...
            {cfg.heroSecondaryCtaEnabled && (
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-full border-2 text-sm font-bold px-8 py-4 theme-cta-btn"
      ...
      <section className="relative overflow-hidden" style={sectionStyle}>
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {cfg.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={cfg.heroTextTheme === 'dark' ? 'text-[#1a1a1a]' : 'text-white'} />}
          </div>
        ) : isSplitHero ? (
    ...
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14">
        <div className="flex items-end justify-between mb-8">
          <h2 className="font-black text-3xl text-[#1a1a1a] tracking-tight">{t.home.allProductsHeading}</h2>
          <Link
            href={`/products`}
        ...
        {products.length === 0 ? (
          <p className="text-[#a89a90] text-sm py-20 text-center">{t.home.noProductsYet}</p>
        ) : (
