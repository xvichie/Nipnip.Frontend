...
export function Home({
  ...
  function buildHero(t: Required<ThemeConfig>) {
    ...
    const sectionStyle = sectionBanner
      ? t.bannerType === 'image'
        ? {
            backgroundImage: `linear-gradient(rgba(244,237,227,0.75),rgba(244,237,227,0.92)), url(${t.bannerUrl})`,
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {t.heroEyebrow && (
          <p className={`italic font-serif ${HERO_EYEBROW_SIZE_CLASS[t.heroEyebrowSize]} ${getHeroTextColorClass(t, 'eyebrow', 'text-[#8f8274]')} mb-4`}>{t.heroEyebrow}</p>
        )}
        <h1 className={`font-serif tracking-tight leading-[0.95] mb-6 ${getHeroTextColorClass(t, 'headline', 'text-[#2b2420]')} ${HERO_HEADLINE_SIZE_CLASS[t.heroHeadlineSize]}`}>
          {t.heroHeadline || store.name}
        </h1>
        {t.heroSubheadline && (
          <p className={`${getHeroTextColorClass(t, 'subheadline', 'text-[#8f8274]')} ${HERO_SUBHEADLINE_SIZE_CLASS[t.heroSubheadlineSize]} mb-9`}>{t.heroSubheadline}</p>
        )}
        {(t.heroCtaEnabled || t.heroSecondaryCtaEnabled) && (
          <div className={`flex flex-wrap items-center gap-6 ${getHeroButtonRowClass(t)}`}>
            {t.heroCtaEnabled && (
              <Link
            ...
            {t.heroSecondaryCtaEnabled && (
              <Link
                ...
                className={`text-xs uppercase tracking-widest font-medium underline underline-offset-4 transition-opacity hover:opacity-70 ${
                  t.heroTextTheme === 'light' ? 'text-white' : 'text-[#2b2420]'
                }`}
              >
      ...
      <section className="relative overflow-hidden" style={sectionStyle}>
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {t.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={t.heroTextTheme === 'light' ? 'text-white' : 'text-[#2b2420]'} />}
          </div>
        ) : isSplitHero ? (
    ...
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 border-t border-[#2b2420]/10">
        <div className="flex items-end justify-between mb-10">
          <h2 className="font-serif italic text-3xl text-[#2b2420]">ყველა პროდუქტი</h2>
        ...
        {products.length === 0 ? (
          <p className="text-[#8f8274] text-sm py-20 text-center">პროდუქტები ჯერ არ არის.</p>
        ) : (
