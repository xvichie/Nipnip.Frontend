...
export function Home({
  ...
  function buildHero(cfg: Required<ThemeConfig>) {
    ...
    const sectionStyle = sectionBanner
      ? cfg.bannerType === 'image'
        ? {
            backgroundImage: `linear-gradient(rgba(244,237,227,0.75),rgba(244,237,227,0.92)), url(${cfg.bannerUrl})`,
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {heroEyebrow && (
          <p className={`italic font-serif ${HERO_EYEBROW_SIZE_CLASS[cfg.heroEyebrowSize]} ${getHeroTextColorClass(cfg, 'eyebrow', 'text-[#8f8274]')} mb-4`}>{heroEyebrow}</p>
        )}
        <h1 className={`font-serif tracking-tight leading-[0.95] mb-6 ${getHeroTextColorClass(cfg, 'headline', 'text-[#2b2420]')} ${HERO_HEADLINE_SIZE_CLASS[cfg.heroHeadlineSize]}`}>
          {heroHeadline || store.name}
        </h1>
        {heroSubheadline && (
          <p className={`${getHeroTextColorClass(cfg, 'subheadline', 'text-[#8f8274]')} ${HERO_SUBHEADLINE_SIZE_CLASS[cfg.heroSubheadlineSize]} mb-9`}>{heroSubheadline}</p>
        )}
              ...
              <Link
                ...
                className={`inline-flex items-center justify-center gap-2 border-2 text-xs uppercase tracking-widest font-medium px-8 py-3.5 transition-colors ${
                  cfg.heroTextTheme === 'light'
                    ? 'border-white text-white hover:bg-white hover:text-[#2b2420]'
            ...
            {cfg.heroSecondaryCtaEnabled && (
              <Link
                ...
                className={`text-xs uppercase tracking-widest font-medium underline underline-offset-4 transition-opacity hover:opacity-70 ${
                  cfg.heroTextTheme === 'light' ? 'text-white' : 'text-[#2b2420]'
                }`}
              >
      ...
      <section className="relative overflow-hidden" style={sectionStyle}>
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {cfg.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={cfg.heroTextTheme === 'light' ? 'text-white' : 'text-[#2b2420]'} />}
          </div>
        ) : isSplitHero ? (
  ...
  const categoriesSection = landingCategories.length > 0 && (
    <section key="categories" className="border-t border-[#2b2420]/10" style={getSectionBackgroundStyle(tokens, 'categories')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
        <div className={`grid ${LANDING_CATEGORY_GRID_CLASS[tokens.landingCategoryColumns]} gap-6`}>
          {landingCategories.map(category => (
            ...
            >
              <CategoryIcon iconUrl={category.iconUrl} iconKey={category.iconKey} iconEmoji={category.iconEmoji} className="w-5 h-5 text-[#2b2420]" />
              <span className="text-xs uppercase tracking-widest text-[#2b2420] group-hover:underline underline-offset-4">
  ...
  const productsSection = (
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 border-t border-[#2b2420]/10">
        <div className="flex items-end justify-between mb-10">
          <h2 className="font-serif italic text-3xl text-[#2b2420]">{t.home.allProductsHeading}</h2>
          <Link href={`/products`} className="text-xs uppercase tracking-widest text-[#2b2420] hover:underline underline-offset-4">
            {t.home.viewAll}
          </Link>
        </div>

        {products.length === 0 ? (
          <p className="text-[#8f8274] text-sm py-20 text-center">{t.home.noProductsYet}</p>
        ) : (
  ...
  const collectionsSection = landingCollections.length > 0 && (
    <section key="collections" className="border-t border-[#2b2420]/10" style={getSectionBackgroundStyle(tokens, 'collections')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 flex flex-col gap-16">
        {landingCollections.map(collection => (
          <ProductScrollRow
            ...
            titleClassName="font-serif italic text-3xl text-[#2b2420]"
            viewAllClassName="text-xs uppercase tracking-widest text-[#2b2420] hover:underline underline-offset-4"
            emptyMessageClassName="text-[#8f8274] text-sm py-16 text-center"
          />
        ))}
      </div>
    </section>
  ...
  const contentSection = hasContentBlock(tokens) && (
    <section key="content" className="border-t border-[#2b2420]/10" style={getSectionBackgroundStyle(tokens, 'content')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
        <ContentBlock
          tokens={tokens}
          lang={lang}
          headingClassName="font-serif italic text-2xl sm:text-3xl text-[#2b2420] mb-5"
          bodyClassName="text-[#8f8274] text-sm sm:text-base leading-relaxed mb-8"
          buttonClassName="inline-flex items-center gap-2 border-2 border-[#2b2420] text-[#2b2420] text-xs uppercase tracking-widest font-medium px-8 py-3.5 hover:bg-[#2b2420] hover:text-white transition-colors"
        />
      </div>
    </section>
  )
  ...
  const faqSection = tokens.showFaqSection && tokens.faqItems.length > 0 && (
    <section key="faq" className="border-t border-[#2b2420]/10" style={getSectionBackgroundStyle(tokens, 'faq')}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-16">
        <FaqAccordion
          tokens={tokens}
          t={t}
          lang={lang}
          headingClassName="font-serif italic text-2xl sm:text-3xl text-[#2b2420] mb-6"
          questionClassName="text-sm text-[#2b2420]"
          answerClassName="text-sm text-[#8f8274] leading-relaxed"
          borderClassName="divide-[#2b2420]/10"
        />
      </div>
    </section>
  )
  ...
  return (
    <div className="bg-[#f4ede3] min-h-screen">
      {getHomeSectionOrder(tokens).map(key => {
        ...
        return (
          <section key={key} className="border-t border-[#2b2420]/10" style={{ backgroundColor: section.backgroundColor || undefined }}>
            <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
              <CustomSectionBlock
                section={section}
                lang={lang}
