  ...
  return (
    <div className="bg-[#f7ede0] min-h-screen">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 pb-24">
        <div className="mb-10">
          <p className="text-xs uppercase tracking-[0.3em] text-[#b5651d] mb-2">კოლექცია</p>
          <h1 className="font-serif text-3xl text-[#3b2418]">{activeCategoryName ?? activeCollectionName ?? 'ყველა პროდუქტი'}</h1>
        </div>
          ...
          <aside className="w-full md:w-52 shrink-0 flex flex-col gap-4 md:sticky md:top-20 md:self-start md:max-h-[calc(100vh-6rem)] md:overflow-y-auto md:pb-6">
            {displayCategories.length > 0 && (
              <div className="border border-[#3b2418]/10 p-5 flex flex-col gap-1">
                <p className="text-xs uppercase tracking-widest text-[#a68a6d] mb-2">კატეგორიები</p>
                <Link
                  href={`/products`}
                  className={[
          ...
          <div className="flex-1">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
              <p className="text-xs uppercase tracking-widest text-[#a68a6d]">
                <span className="text-[#3b2418] font-medium">{totalCount}</span> პროდუქტი
              </p>
            ...
            {isLoading ? (
              ...
              <div className="py-32 flex flex-col items-center gap-4 text-center">
                <p className="text-[#a68a6d] text-sm">პროდუქტი ვერ მოიძებნა.</p>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-6">
