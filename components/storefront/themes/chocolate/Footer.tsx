    ...
    <footer className="bg-[#f7ede0] border-t border-[#3b2418]/10 mt-auto">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 flex flex-col gap-6">
          ...
          <div className="flex flex-wrap gap-x-10 gap-y-6 pb-6 border-b border-[#3b2418]/10">
            {tokens.footerLinkColumns.map((column, i) => (
              <div key={i} className="flex flex-col gap-2 min-w-[100px]">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-[#a68a6d]">{column.title}</p>
                {column.links.map((link, j) => (
                  <a key={j} href={link.url} className="text-xs uppercase tracking-widest text-[#a68a6d] hover:text-[#3b2418] transition-colors">{link.label}</a>
                ))}
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
          <p className="text-[#a68a6d] text-xs tracking-wide">{tokens.footerCopyrightText || `© ${new Date().getFullYear()} ${storeName}`}</p>
          <div className="flex items-center gap-5">
            ...
            {tokens.showPlatformAttribution && <p className="text-[#a68a6d] text-xs tracking-wide">შექმნილია NipNip-ის მიერ</p>}
          </div>
        </div>
      </div>
