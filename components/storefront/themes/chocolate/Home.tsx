...
export function Home({
  ...
  function buildHero(t: Required<ThemeConfig>) {
    ...
    const sectionStyle = sectionBanner
      ? t.bannerType === 'image'
        ? {
            backgroundImage: `linear-gradient(rgba(247,237,224,0.8),rgba(247,237,224,0.92)), url(${t.bannerUrl})`,
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {t.heroEyebrow && (
          <p className={`${HERO_EYEBROW_SIZE_CLASS[t.heroEyebrowSize]} uppercase tracking-[0.3em] ${getHeroTextColorClass(t, 'eyebrow', 'text-[#b5651d]')} mb-4`}>{t.heroEyebrow}</p>
        )}
        <h1 className={`font-serif tracking-tight mb-6 ${getHeroTextColorClass(t, 'headline', 'text-[#3b2418]')} ${HERO_HEADLINE_SIZE_CLASS[t.heroHeadlineSize]}`}>
          {t.heroHeadline || store.name}
        </h1>
        {t.heroSubheadline && (
          <p className={`${getHeroTextColorClass(t, 'subheadline', 'text-[#7a5c47]')} ${HERO_SUBHEADLINE_SIZE_CLASS[t.heroSubheadlineSize]} mb-9`}>{t.heroSubheadline}</p>
        )}
      ...
      <section className="relative overflow-hidden" style={sectionStyle}>
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {t.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={t.heroTextTheme === 'dark' ? 'text-[#3b2418]' : 'text-white'} />}
          </div>
        ) : isSplitHero ? (
    ...
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
        <div className="flex items-end justify-between mb-8">
          <h2 className="font-serif text-2xl text-[#3b2418]">ყველა პროდუქტი</h2>
        ...
        {products.length === 0 ? (
          <p className="text-[#a68a6d] text-sm py-20 text-center">პროდუქტები ჯერ არ არის.</p>
        ) : (
