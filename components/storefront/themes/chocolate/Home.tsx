...
export function Home({
  ...
  function buildHero(cfg: Required<ThemeConfig>) {
    ...
    const sectionStyle = sectionBanner
      ? cfg.bannerType === 'image'
        ? {
            backgroundImage: `linear-gradient(rgba(247,237,224,0.8),rgba(247,237,224,0.92)), url(${cfg.bannerUrl})`,
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {heroEyebrow && (
          <p className={`${HERO_EYEBROW_SIZE_CLASS[cfg.heroEyebrowSize]} uppercase tracking-[0.3em] ${getHeroTextColorClass(cfg, 'eyebrow', 'text-[#b5651d]')} mb-4`}>{heroEyebrow}</p>
        )}
        <h1 className={`font-serif tracking-tight mb-6 ${getHeroTextColorClass(cfg, 'headline', 'text-[#3b2418]')} ${HERO_HEADLINE_SIZE_CLASS[cfg.heroHeadlineSize]}`}>
          {heroHeadline || store.name}
        </h1>
        {heroSubheadline && (
          <p className={`${getHeroTextColorClass(cfg, 'subheadline', 'text-[#7a5c47]')} ${HERO_SUBHEADLINE_SIZE_CLASS[cfg.heroSubheadlineSize]} mb-9`}>{heroSubheadline}</p>
        )}
          ...
          <div className={`flex flex-wrap items-center gap-6 ${getHeroButtonRowClass(cfg)}`}>
            ...
            {cfg.heroSecondaryCtaEnabled && (
              <Link
                ...
                className="text-xs uppercase tracking-widest underline underline-offset-4 transition-opacity hover:opacity-70"
    ...
    const heroImage = (
      <div className={`relative h-full ${imageOrderClass} ${heroImageHiddenMobile ? 'hidden md:block' : ''}`}>
        ...
        <div className={`relative h-full flex items-center justify-center overflow-hidden ${imageBanner || sectionBanner ? '' : 'bg-white border border-[#3b2418]/10'}`}>
      ...
      <section className="relative overflow-hidden" style={sectionStyle}>
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {cfg.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={cfg.heroTextTheme === 'dark' ? 'text-[#3b2418]' : 'text-white'} />}
          </div>
        ) : isSplitHero ? (
  ...
  const categoriesSection = landingCategories.length > 0 && (
    <section key="categories" className="border-t border-[#3b2418]/10" style={getSectionBackgroundStyle(tokens, 'categories')}>
        ...
        <div className={`grid ${LANDING_CATEGORY_GRID_CLASS[tokens.landingCategoryColumns]} gap-4`}>
          {landingCategories.map(category => (
            <Link
              ...
              className="flex flex-col items-center justify-center gap-2.5 border border-[#3b2418]/10 hover:border-[#3b2418]/30 transition-colors px-4 py-7 text-center text-xs uppercase tracking-widest text-[#3b2418]/70"
            >
    ...
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
        <div className="flex items-end justify-between mb-8">
          <h2 className="font-serif text-2xl text-[#3b2418]">{t.home.allProductsHeading}</h2>
        ...
        {products.length === 0 ? (
          <p className="text-[#a68a6d] text-sm py-20 text-center">{t.home.noProductsYet}</p>
        ) : (
  ...
  const collectionsSection = landingCollections.length > 0 && (
    <section key="collections" className="border-t border-[#3b2418]/10" style={getSectionBackgroundStyle(tokens, 'collections')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 flex flex-col gap-16">
        {landingCollections.map(collection => (
          <ProductScrollRow
            ...
            titleClassName="font-serif text-2xl text-[#3b2418]"
            viewAllClassName="text-xs uppercase tracking-widest hover:underline underline-offset-4"
            viewAllStyle={{ color: tokens.accentColor }}
            emptyMessageClassName="text-[#a68a6d] text-sm py-16 text-center"
          />
        ))}
      </div>
    </section>
  ...
  const contentSection = hasContentBlock(tokens) && (
    <section key="content" className="border-t border-[#3b2418]/10" style={getSectionBackgroundStyle(tokens, 'content')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
        <ContentBlock
          tokens={tokens}
          lang={lang}
          headingClassName="font-serif text-2xl sm:text-3xl text-[#3b2418] tracking-tight mb-5"
          bodyClassName="text-[#7a5c47] text-sm sm:text-base leading-relaxed mb-7"
  ...
  const faqSection = tokens.showFaqSection && tokens.faqItems.length > 0 && (
    <section key="faq" className="border-t border-[#3b2418]/10" style={getSectionBackgroundStyle(tokens, 'faq')}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-16">
        <FaqAccordion
          tokens={tokens}
          t={t}
          lang={lang}
          headingClassName="font-serif text-2xl sm:text-3xl text-[#3b2418] tracking-tight mb-6"
          questionClassName="text-sm text-[#3b2418]"
          answerClassName="text-sm text-[#7a5c47] leading-relaxed"
          borderClassName="divide-[#3b2418]/10"
        />
      </div>
    </section>
  )
  ...
  return (
    <div className="bg-[#f7ede0] min-h-screen">
      {getHomeSectionOrder(tokens).map(key => {
        ...
        return (
          <section key={key} className="border-t border-[#3b2418]/10" style={{ backgroundColor: section.backgroundColor || undefined }}>
            <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
              <CustomSectionBlock
                section={section}
                lang={lang}
