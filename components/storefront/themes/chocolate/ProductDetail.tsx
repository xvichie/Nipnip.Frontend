    ...
    <div className="bg-[#f7ede0] min-h-screen">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 pb-20">
        <Breadcrumbs
          ...
          t={t}
          textClassName="text-[#3b2418]"
          mutedClassName="text-[#a68a6d] hover:text-[#3b2418]"
        />
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-14">
          ...
          <div className="flex gap-3">
              ...
              <div className="flex flex-col gap-2 shrink-0">
                {images.map((image, i) => (
                  <button
                    ...
                    style={{ borderColor: activeImage === i ? tokens.accentColor : 'rgba(59,36,24,0.12)' }}
                  >
            ...
            <div className="flex-1 aspect-[4/5] bg-white border border-[#3b2418]/10 overflow-hidden">
              {images[activeImage] ? (
                <button
                  type="button"
                ...
                <div className="w-full h-full flex items-center justify-center text-[#a68a6d] text-[10px] uppercase tracking-widest">{t.product.noImage}</div>
              )}
            </div>
          </div>

          <div className="flex flex-col">
            <h1 className="font-serif text-3xl sm:text-4xl text-[#3b2418] tracking-tight leading-tight mb-5">
              {productName}
            </h1>
            ...
            {salePrice !== null ? (
              <div className="flex items-center gap-3 mb-8">
                ...
                <span className="text-[#a68a6d] text-lg line-through">₾{price.toFixed(2)}</span>
              </div>
            ) : (
            ...
            {sizeGuideContent && (
              <button
                ...
                className="text-xs uppercase tracking-widest text-[#a68a6d] hover:text-[#3b2418] underline underline-offset-4 mb-5 text-left w-fit"
              >
                {t.product.sizeGuide}
              </button>
            )}
            ...
            <div className="flex items-center gap-4 mb-5">
              <div className="flex items-center border border-[#3b2418]/15">
                <button
                  type="button"
                  onClick={() => setQuantity(q => Math.max(1, q - 1))}
                  className="w-9 h-10 flex items-center justify-center text-[#3b2418]/60 hover:text-[#3b2418] transition-colors text-lg"
                >
                  −
                </button>
                <span className="w-10 text-center text-[#3b2418] text-sm font-medium tabular-nums">{quantity}</span>
                <button
                  type="button"
              ...
              {selectionComplete && stock !== null && (
                <span className={`text-xs uppercase tracking-widest ${isLowStock ? 'text-red-600 font-medium' : 'text-[#a68a6d]'}`}>
            ...
            <button
              ...
              className="w-full py-4 text-xs uppercase tracking-[0.2em] font-medium mb-9 disabled:opacity-40 disabled:cursor-not-allowed theme-cta-btn"
              style={{ backgroundColor: added ? '#2d6a2d' : tokens.accentColor, color: tokens.buttonTextColor, '--btn-hover-bg': added ? shadeColor('#2d6a2d', -15) : getButtonHoverColor(tokens) } as React.CSSProperties}
            >
              {added
            ...
            {addError && (
              <p className="text-red-500 text-sm -mt-7 mb-8">{t.product.addToCartError}</p>
            )}

            {deliveryEstimateText && (
              <p className="text-xs uppercase tracking-widest text-[#a68a6d] mb-4">{deliveryEstimateText}</p>
            )}

            {tokens.trustBadges.length > 0 && (
              <div className="flex flex-wrap gap-x-4 gap-y-1 mb-8">
                {tokens.trustBadges.map((badge, i) => (
                  <span key={i} className="text-xs uppercase tracking-widest text-[#a68a6d]">✓ {getTrustBadgeText(badge, lang)}</span>
                ))}
              </div>
            )}

            {productDescription && (
              <div className="pt-7 border-t border-[#3b2418]/10">
                <p className="text-xs uppercase tracking-widest text-[#a68a6d] mb-3">{t.product.description}</p>
                <p className="text-[#7a5c47] text-sm leading-relaxed">{productDescription}</p>
              </div>
            )}

            {product.videoUrl && (
              <div className="pt-7 border-t border-[#3b2418]/10">
                <p className="text-xs uppercase tracking-widest text-[#a68a6d] mb-3">{t.product.video}</p>
        ...
        {tokens.showRelatedProducts && product.relatedProducts.length > 0 && (
          <div className="mt-16 pt-10 border-t border-[#3b2418]/10">
            <p className="text-xs uppercase tracking-widest text-[#a68a6d] mb-6">{relatedProductsHeading || t.product.relatedProducts}</p>
