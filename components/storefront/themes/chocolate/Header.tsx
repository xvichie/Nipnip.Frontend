...
export function Header({
  ...
  return (
    <header
      className={`${tokens.headerSticky ? 'sticky top-0' : ''} z-50 bg-[#f7ede0] border-b border-[#3b2418]/10`}
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-20 flex items-center justify-between gap-6">
          ...
          {tokens.logoUrl ? (
            <>
              ...
              {tokens.showStoreName && (
                <span className="font-serif text-[#3b2418] text-lg tracking-[0.15em] uppercase truncate">{storeName}</span>
              )}
            </>
          ) : (
            <span className="font-serif text-[#3b2418] text-xl tracking-[0.15em] uppercase truncate">{storeName}</span>
          )}
        </Link>
        ...
        {hasNav && (
          <nav className="hidden md:flex items-center gap-7">
            <Link href={`/products`} className="text-xs uppercase tracking-widest text-[#a68a6d] hover:text-[#3b2418] transition-colors whitespace-nowrap">
              ...
              isDropdown ? (
                <div className="relative" onMouseEnter={openCategoryMenu} onMouseLeave={closeCategoryMenuDelayed}>
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs uppercase tracking-widest text-[#a68a6d] hover:text-[#3b2418] transition-colors whitespace-nowrap"
                  >
                    {t.header.categories}
                  ...
                  {categoriesOpen && (
                    <div className="absolute top-full left-0 mt-3 min-w-[200px] bg-[#f7ede0] shadow-lg border border-[#3b2418]/10 py-2 flex flex-col z-10">
            ...
            {navPages.map(page => (
              <Link
                ...
                className="text-xs uppercase tracking-widest text-[#a68a6d] hover:text-[#3b2418] transition-colors whitespace-nowrap"
              >
                {page.title}
              </Link>
            ))}
        ...
        <div className="flex items-center gap-3 sm:gap-4 shrink-0">
          ...
          {!isCheckout && (
            <Link
              href={`/cart`}
              className="relative flex items-center gap-2 border border-[#3b2418]/20 hover:border-[#3b2418] px-4 py-2 text-xs uppercase tracking-widest text-[#3b2418] transition-colors"
            >
          ...
          {hasNav && (
            <button
              ...
              className="md:hidden flex items-center justify-center w-9 h-9 text-[#a68a6d] hover:text-[#3b2418] transition-colors"
            >
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50">
          ...
          <div className="absolute top-0 right-0 h-full w-72 max-w-[80vw] bg-[#f7ede0] shadow-xl flex flex-col p-6 overflow-y-auto">
            <button
              type="button"
              onClick={() => setMobileOpen(false)}
              aria-label={t.header.closeAriaLabel}
              className="self-end mb-4 w-8 h-8 flex items-center justify-center text-[#a68a6d] hover:text-[#3b2418]"
            >
            ...
            <Link
              href={`/products`}
              onClick={() => setMobileOpen(false)}
              className="text-xs uppercase tracking-widest text-[#3b2418] py-3 border-b border-[#3b2418]/10"
            >
              {t.header.allProducts}
            </Link>
            {navCategories.map(category => (
            ...
            {navPages.map(page => (
              <Link
                ...
                href={`/pages/${page.slug}`}
                onClick={() => setMobileOpen(false)}
                className="text-xs uppercase tracking-widest text-[#a68a6d] py-3 border-b border-[#3b2418]/10"
              >
                {page.title}
              </Link>
            ))}
