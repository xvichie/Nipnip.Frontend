    ...
    <footer className="bg-white border-t border-slate-200 mt-auto">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 flex flex-col gap-4">
          ...
          <div className="flex flex-wrap gap-x-10 gap-y-4 pb-4 border-b border-slate-200">
            {tokens.footerLinkColumns.map((column, i) => (
              <div key={i} className="flex flex-col gap-1.5 min-w-[100px]">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-400">{resolveThemeText(column.title, column.translations?.en, column.translations?.ru, lang)}</p>
                {column.links.map((link, j) => (
                  <a key={j} href={link.url} className="text-xs text-slate-500 hover:text-slate-900 transition-colors">{resolveThemeText(link.label, link.translations?.en, link.translations?.ru, lang)}</a>
                ))}
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2.5">
          <p className="text-slate-400 text-xs">{getThemeText(tokens, 'footerCopyrightText', lang) || `© ${new Date().getFullYear()} ${storeName}`}</p>
          <div className="flex items-center gap-4">
            ...
            {tokens.showPlatformAttribution && <p className="text-slate-400 text-xs">{t.footer.poweredBy}</p>}
          </div>
        </div>
      </div>
