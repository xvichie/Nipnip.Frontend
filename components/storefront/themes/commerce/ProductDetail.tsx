      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 pb-16">
          ...
          <div className="flex flex-col">
            ...
            {sizeGuideContent && (
              <button
                ...
                className="text-xs text-slate-400 hover:text-slate-900 underline underline-offset-2 mb-4 text-left w-fit"
              >
                {t.product.sizeGuide}
              </button>
            )}
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              {selectionComplete && stock !== null && (
                <span className={`text-xs ${isLowStock ? 'text-red-600 font-medium' : 'text-slate-400'}`}>
            ...
            {deliveryEstimateText && (
              <p className="text-xs text-slate-400 mb-4">{deliveryEstimateText}</p>
            )}

            {tokens.trustBadges.length > 0 && (
              <div className="flex flex-wrap gap-x-4 gap-y-1 mb-8">
                {tokens.trustBadges.map((badge, i) => (
                  <span key={i} className="text-xs text-slate-400">✓ {getTrustBadgeText(badge, lang)}</span>
                ))}
              </div>
            )}
            ...
            {productDescription && (
              <div className="rounded-md border border-slate-200 bg-slate-50 p-4">
                <p className="text-xs font-semibold uppercase tracking-widest text-slate-400 mb-3">{t.product.description}</p>
                <p className="text-slate-600 text-sm leading-relaxed">{productDescription}</p>
              </div>
            )}

            {product.videoUrl && (
              <div className="rounded-md border border-slate-200 bg-slate-50 p-4">
                <p className="text-xs font-semibold uppercase tracking-widest text-slate-400 mb-3">{t.product.video}</p>
        ...
        {tokens.showRelatedProducts && product.relatedProducts.length > 0 && (
          <div className="mt-14 pt-8 border-t border-slate-200">
            <h2 className="font-black text-2xl text-slate-900 tracking-tight mb-6">{relatedProductsHeading || t.product.relatedProducts}</h2>
