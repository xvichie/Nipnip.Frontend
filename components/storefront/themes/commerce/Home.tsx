  ...
  function buildHero(cfg: Required<ThemeConfig>) {
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {heroEyebrow && (
          <p className={`flex items-center gap-2 ${HERO_EYEBROW_SIZE_CLASS[cfg.heroEyebrowSize]} font-semibold uppercase tracking-widest ${getHeroTextColorClass(cfg, 'eyebrow', 'text-slate-500')} mb-3`}>
            <span className="inline-block w-1.5 h-1.5 shrink-0" style={{ backgroundColor: cfg.accentColor }} aria-hidden />
            {heroEyebrow}
          </p>
        )}
        <h1 className={`font-black tracking-tight mb-5 ${getHeroTextColorClass(cfg, 'headline', 'text-slate-900')} ${HERO_HEADLINE_SIZE_CLASS[cfg.heroHeadlineSize]}`}>
          {heroHeadline || store.name}
        </h1>
        {heroSubheadline && (
          <p className={`${getHeroTextColorClass(cfg, 'subheadline', 'text-slate-500')} ${HERO_SUBHEADLINE_SIZE_CLASS[cfg.heroSubheadlineSize]} mb-8`}>{heroSubheadline}</p>
        )}
      ...
      <section className="bg-white relative overflow-hidden" style={sectionStyle}>
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {cfg.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={cfg.heroTextTheme === 'dark' ? 'text-slate-900' : 'text-white'} />}
          </div>
        ) : isSplitHero ? (
  ...
  const productsSection = (
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
        ...
        {products.length === 0 ? (
          <p className="text-slate-400 text-sm py-20 text-center">{t.home.noProductsYet}</p>
        ) : (
  ...
  return (
    <div className="bg-white min-h-screen">
      {getHomeSectionOrder(tokens).map(key => {
        ...
        return (
          <section key={key} className="border-t border-slate-200" style={{ backgroundColor: section.backgroundColor || undefined }}>
            <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
              <CustomSectionBlock
                section={section}
                lang={lang}
