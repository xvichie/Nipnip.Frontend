    ...
    <footer className="bg-[#0a0a0a] border-t border-white/10 mt-auto">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 flex flex-col gap-6">
          ...
          <div className="flex flex-wrap gap-x-10 gap-y-6 pb-6 border-b border-white/10">
            {tokens.footerLinkColumns.map((column, i) => (
              <div key={i} className="flex flex-col gap-2 min-w-[100px]">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-white/40">{column.title}</p>
                {column.links.map((link, j) => (
                  <a key={j} href={link.url} className="text-xs text-white/40 hover:text-white transition-colors">{link.label}</a>
                ))}
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-white/40 text-xs">{tokens.footerCopyrightText || `© ${new Date().getFullYear()} ${storeName}`}</p>
          <div className="flex flex-wrap items-center justify-center gap-4">
            ...
            {tokens.showPlatformAttribution && <p className="text-white/40 text-xs">შექმნილია NipNip-ის მიერ</p>}
          </div>
        </div>
      </div>
