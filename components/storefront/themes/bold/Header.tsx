      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-6">
        <Link href={`/store/${slug}`} className="flex items-center gap-3 shrink-0">
          {tokens.logoUrl ? (
            <>
              ...
              <img src={tokens.logoUrl} alt={storeName} className="h-8 w-auto object-contain" />
              {tokens.showStoreName && (
                <span className="font-black text-white text-lg tracking-tight">{storeName}</span>
              )}
            </>
          ) : (
            <span className="font-black text-white text-lg tracking-tight">{storeName}</span>
        ...
        {hasNav && (
          <nav className="hidden md:flex items-center gap-6">
            <Link href={`/store/${slug}/products`} className="text-sm font-medium text-white/60 hover:text-white transition-colors whitespace-nowrap">
              ყველა პროდუქტი
            </Link>
                    ...
                    <div className="absolute top-full left-0 mt-2 min-w-[190px] rounded-xl bg-[#0a0a0a] shadow-xl border border-white/10 py-1.5 flex flex-col z-10">
                      {navCategories.map(category => (
                        <Link
                          ...
                          className="px-4 py-2 text-sm font-medium text-white/60 hover:text-white hover:bg-white/5 transition-colors whitespace-nowrap"
                        >
                          {category.name}
                        </Link>
                      ))}
