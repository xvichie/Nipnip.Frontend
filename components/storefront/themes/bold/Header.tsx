    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-6">
        ...
        {hasNav && (
          <nav className="hidden md:flex items-center gap-6">
            <Link href={`/products`} className="text-sm font-medium text-white/60 hover:text-white transition-colors whitespace-nowrap">
              ...
              isDropdown ? (
                ...
                >
                  <button
                    ...
                    className="flex items-center gap-1 text-sm font-medium text-white/60 hover:text-white transition-colors whitespace-nowrap"
                  >
                    {t.header.categories}
                  ...
                  {categoriesOpen && (
                    <div className="absolute top-full left-0 mt-2 min-w-[190px] rounded-xl bg-[#0a0a0a] shadow-xl border border-white/10 py-1.5 flex flex-col z-10">
                      {navCategories.map(category => (
                        <Link
                          key={category.id}
                          href={`/products/category/${category.slug}`}
                          onClick={() => setCategoriesOpen(false)}
                          className={`py-2 text-sm font-medium text-white/60 hover:text-white hover:bg-white/5 transition-colors whitespace-nowrap ${category.isChild ? 'pl-8 pr-4' : 'px-4'}`}
                        >
                ...
                navCategories.map(category => (
                  <Link
                    key={category.id}
                    href={`/products/category/${category.slug}`}
                    className={`text-sm font-medium text-white/60 hover:text-white transition-colors whitespace-nowrap ${category.isChild ? 'pl-3' : ''}`}
                  >
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          {!isCheckout && (
            <Link
              href={`/cart`}
              className="relative flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold text-white bg-white/10 hover:bg-white/15 transition-colors"
            >
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50">
          ...
          <div className="absolute top-0 right-0 h-full w-72 max-w-[80vw] bg-[#0a0a0a] border-l border-white/10 shadow-xl flex flex-col p-5 overflow-y-auto">
            <button
              type="button"
            ...
            <Link
              href={`/products`}
              onClick={() => setMobileOpen(false)}
              className="text-sm font-semibold text-white py-2.5 border-b border-white/10"
            >
              {t.header.allProducts}
            </Link>
            {navCategories.map(category => (
              <Link
                ...
                href={`/products/category/${category.slug}`}
                onClick={() => setMobileOpen(false)}
                className={`text-sm font-medium text-white/60 py-2.5 border-b border-white/10 ${category.isChild ? 'pl-4' : ''}`}
              >
                {getCategoryName(category, lang)}
              </Link>
            ))}
            {navPages.map(page => (
              <Link
                ...
                href={`/pages/${page.slug}`}
                onClick={() => setMobileOpen(false)}
                className="text-sm font-medium text-white/60 py-2.5 border-b border-white/10"
              >
                {getPageTitle(page, lang)}
              </Link>
            ))}
