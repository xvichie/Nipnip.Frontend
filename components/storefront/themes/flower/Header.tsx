      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-6">
          ...
          {tokens.logoUrl ? (
            <>
              ...
              {tokens.showStoreName && (
                <span className="font-bold text-[#3d2b28] text-lg truncate">{storeName}</span>
              )}
            </>
          ) : (
            <span className="font-bold text-[#3d2b28] text-lg truncate">{storeName}</span>
        ...
        {hasNav && (
          <nav className="hidden md:flex items-center gap-5">
            <Link href={`/products`} className="text-sm text-[#8a7169] hover:text-[#3d2b28] transition-colors whitespace-nowrap">
              ...
              isDropdown ? (
                ...
                >
                  <button
                    ...
                    className="flex items-center gap-1 text-sm text-[#8a7169] hover:text-[#3d2b28] transition-colors whitespace-nowrap"
                  >
                    {t.header.categories}
                  ...
                  {categoriesOpen && (
                    <div className="absolute top-full left-0 mt-2 min-w-[180px] rounded-2xl bg-white shadow-lg border border-[#f3e3de] py-1.5 flex flex-col z-10">
                      {navCategories.map(category => (
                        <Link
                          key={category.id}
                          href={`/products/category/${category.slug}`}
                          onClick={() => setCategoriesOpen(false)}
                          className={`py-2 text-sm text-[#8a7169] hover:text-[#3d2b28] hover:bg-[#fbf1ec] transition-colors whitespace-nowrap ${category.isChild ? 'pl-8 pr-4' : 'px-4'}`}
                        >
                ...
                navCategories.map(category => (
                  <Link
                    key={category.id}
                    href={`/products/category/${category.slug}`}
                    className={`text-sm text-[#8a7169] hover:text-[#3d2b28] transition-colors whitespace-nowrap ${category.isChild ? 'pl-3' : ''}`}
                  >
            ...
            {navPages.map(page => (
              <Link
                ...
                className="text-sm text-[#8a7169] hover:text-[#3d2b28] transition-colors whitespace-nowrap"
              >
                {page.title}
              </Link>
            ))}
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          {!isCheckout && (
            <Link
              href={`/cart`}
              className="relative flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium text-white transition-transform hover:scale-105"
          ...
          {hasNav && (
            <button
              ...
              className="md:hidden flex items-center justify-center w-9 h-9 rounded-full text-[#8a7169] hover:text-[#3d2b28] hover:bg-[#fbf1ec] transition-colors"
            >
        ...
        <div className="md:hidden fixed inset-0 z-50">
          ...
          <div className="absolute top-0 right-0 h-full w-72 max-w-[80vw] bg-white shadow-xl flex flex-col p-5 overflow-y-auto">
            <button
              ...
              className="self-end mb-3 w-8 h-8 flex items-center justify-center text-[#8a7169] hover:text-[#3d2b28]"
            >
            ...
            <Link
              href={`/products`}
              onClick={() => setMobileOpen(false)}
              className="text-sm font-medium text-[#3d2b28] py-2.5 border-b border-[#f3e3de]"
            >
              {t.header.allProducts}
            </Link>
            {navCategories.map(category => (
              <Link
                ...
                href={`/products/category/${category.slug}`}
                onClick={() => setMobileOpen(false)}
                className={`text-sm text-[#8a7169] py-2.5 border-b border-[#f3e3de] ${category.isChild ? 'pl-4' : ''}`}
              >
                {getCategoryName(category, lang)}
              </Link>
            ))}
            {navPages.map(page => (
              <Link
                ...
                href={`/pages/${page.slug}`}
                onClick={() => setMobileOpen(false)}
                className="text-sm text-[#8a7169] py-2.5 border-b border-[#f3e3de]"
              >
                {page.title}
              </Link>
            ))}
