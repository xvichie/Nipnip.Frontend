...
import { getBannerBackgroundStyle, getCustomSectionId, getHeroBackgroundImageClass, getHeroButtonRowClass, getHeroCtaHref, getHeroCtaLabel, getHeroImageClass, getHeroOverlayStyle, getHeroTextAlignClass, getHeroTextColorClass, getHomeSectionOrder, getSectionBackgroundStyle, hasBanner, hasContentBlock, hasHeroVideo, HERO_EYEBROW_SIZE_CLASS, HERO_HEADLINE_SIZE_CLASS, HERO_HEIGHT_CLASS, HERO_SUBHEADLINE_SIZE_CLASS, HERO_TEXT_POSITION_CLASS, LANDING_CATEGORY_GRID_CLASS, getHeroSecondaryCtaHref, getHeroSecondaryCtaLabel, getHeroSlideTranslations } from '@/lib/store/theme-config'
...
export function Home({
  ...
  function buildHero(cfg: Required<ThemeConfig>) {
    ...
    const sectionStyle = sectionBanner
      ? cfg.bannerType === 'image'
        ? {
            backgroundImage: `linear-gradient(rgba(253,246,242,0.75),rgba(253,246,242,0.92)), url(${cfg.bannerUrl})`,
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {heroEyebrow && (
          <p
            className={`inline-block ${HERO_EYEBROW_SIZE_CLASS[cfg.heroEyebrowSize]} font-semibold uppercase tracking-widest px-3 py-1 rounded-full mb-4`}
            style={{ backgroundColor: `${cfg.accentColor}18`, color: cfg.accentColor }}
          >
            {heroEyebrow}
          </p>
        )}
        <h1 className={`font-bold tracking-tight mb-5 ${getHeroTextColorClass(cfg, 'headline', 'text-[#3d2b28]')} ${HERO_HEADLINE_SIZE_CLASS[cfg.heroHeadlineSize]}`}>
          {heroHeadline || store.name}
        </h1>
        {heroSubheadline && (
          <p className={`${getHeroTextColorClass(cfg, 'subheadline', 'text-[#8a7169]')} ${HERO_SUBHEADLINE_SIZE_CLASS[cfg.heroSubheadlineSize]} mb-8`}>{heroSubheadline}</p>
        )}
          ...
          <div className={`flex flex-wrap items-center gap-3 ${getHeroButtonRowClass(cfg)}`}>
            {cfg.heroCtaEnabled && (
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-full text-white text-sm font-semibold px-7 py-3 shadow-sm hover:shadow-md transition-shadow"
            ...
            {cfg.heroSecondaryCtaEnabled && (
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-full border text-sm font-semibold px-7 py-3 transition-colors"
    ...
    const heroImage = (
      <div className={`relative h-full ${imageOrderClass} ${heroImageHiddenMobile ? 'hidden md:block' : ''}`}>
        ...
        <div className={`relative h-full flex items-center justify-center overflow-hidden rounded-2xl ${imageBanner || sectionBanner ? '' : 'bg-white shadow-sm'}`}>
    ...
    return (
      <section className="bg-[#fdf6f2] relative overflow-hidden" style={sectionStyle}>
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {cfg.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={cfg.heroTextTheme === 'dark' ? 'text-[#3d2b28]' : 'text-white'} />}
          </div>
        ) : isSplitHero ? (
  ...
  const categoriesSection = landingCategories.length > 0 && (
    <section key="categories" className="border-t border-[#f3e3de]" style={getSectionBackgroundStyle(tokens, 'categories')}>
        ...
        <div className={`grid ${LANDING_CATEGORY_GRID_CLASS[tokens.landingCategoryColumns]} gap-3`}>
          {landingCategories.map(category => (
            <Link
              ...
              className="flex flex-col items-center justify-center gap-2 rounded-2xl bg-white shadow-sm hover:shadow-md transition-shadow px-4 py-6 text-center text-sm font-medium text-[#5a453f]"
            >
    ...
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
        <div className="flex items-end justify-between mb-6">
          <h2 className="font-bold text-2xl text-[#3d2b28]">{t.home.allProductsHeading}</h2>
          <Link href={`/products`} className="text-sm font-medium hover:underline" style={{ color: tokens.accentColor }}>
            {t.home.viewAll}
          </Link>
        </div>

        {products.length === 0 ? (
          <p className="text-[#cbb3aa] text-sm py-20 text-center">{t.home.noProductsYet}</p>
        ) : (
  ...
  const collectionsSection = landingCollections.length > 0 && (
    <section key="collections" className="border-t border-[#f3e3de]" style={getSectionBackgroundStyle(tokens, 'collections')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14 flex flex-col gap-14">
        {landingCollections.map(collection => (
          <ProductScrollRow
            ...
            titleClassName="font-bold text-2xl text-[#3d2b28]"
            viewAllClassName="text-sm font-medium hover:underline"
            viewAllStyle={{ color: tokens.accentColor }}
            emptyMessageClassName="text-[#cbb3aa] text-sm py-16 text-center"
          />
        ))}
      </div>
    </section>
  ...
  const contentSection = hasContentBlock(tokens) && (
    <section key="content" className="border-t border-[#f3e3de]" style={getSectionBackgroundStyle(tokens, 'content')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
        <ContentBlock
          tokens={tokens}
          lang={lang}
          headingClassName="font-bold text-2xl sm:text-3xl text-[#3d2b28] mb-4"
          bodyClassName="text-[#8a7169] text-sm sm:text-base leading-relaxed mb-6"
          buttonClassName="inline-flex items-center gap-2 rounded-full text-white text-sm font-semibold px-7 py-3 shadow-sm hover:shadow-md transition-shadow"
  ...
  const faqSection = tokens.showFaqSection && tokens.faqItems.length > 0 && (
    <section key="faq" className="border-t border-[#f3e3de]" style={getSectionBackgroundStyle(tokens, 'faq')}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-14">
        <FaqAccordion
          tokens={tokens}
          t={t}
          lang={lang}
          headingClassName="font-bold text-2xl sm:text-3xl text-[#3d2b28] mb-6"
          questionClassName="font-medium text-sm text-[#3d2b28]"
          answerClassName="text-sm text-[#8a7169] leading-relaxed"
          borderClassName="divide-[#f3e3de]"
        />
      </div>
    </section>
  )
  ...
  return (
    <div className="bg-[#fdf6f2] min-h-screen">
      {getHomeSectionOrder(tokens).map(key => {
        ...
        return (
          <section key={key} className="border-t border-[#f3e3de]" style={{ backgroundColor: section.backgroundColor || undefined }}>
            <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
              <CustomSectionBlock
                section={section}
                lang={lang}
