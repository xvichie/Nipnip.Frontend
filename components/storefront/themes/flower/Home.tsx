...
export function Home({
  ...
  function buildHero(t: Required<ThemeConfig>) {
    ...
    const sectionStyle = sectionBanner
      ? t.bannerType === 'image'
        ? {
            backgroundImage: `linear-gradient(rgba(253,246,242,0.75),rgba(253,246,242,0.92)), url(${t.bannerUrl})`,
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {t.heroEyebrow && (
          <p
            className={`inline-block ${HERO_EYEBROW_SIZE_CLASS[t.heroEyebrowSize]} font-semibold uppercase tracking-widest px-3 py-1 rounded-full mb-4`}
            style={{ backgroundColor: `${t.accentColor}18`, color: t.accentColor }}
          >
            {t.heroEyebrow}
          </p>
        )}
        <h1 className={`font-bold tracking-tight mb-5 ${getHeroTextColorClass(t, 'headline', 'text-[#3d2b28]')} ${HERO_HEADLINE_SIZE_CLASS[t.heroHeadlineSize]}`}>
          {t.heroHeadline || store.name}
        </h1>
        {t.heroSubheadline && (
          <p className={`${getHeroTextColorClass(t, 'subheadline', 'text-[#8a7169]')} ${HERO_SUBHEADLINE_SIZE_CLASS[t.heroSubheadlineSize]} mb-8`}>{t.heroSubheadline}</p>
        )}
      ...
      <section className="bg-[#fdf6f2] relative overflow-hidden" style={sectionStyle}>
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {t.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={t.heroTextTheme === 'dark' ? 'text-[#3d2b28]' : 'text-white'} />}
          </div>
        ) : isSplitHero ? (
    ...
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
        <div className="flex items-end justify-between mb-6">
          <h2 className="font-bold text-2xl text-[#3d2b28]">ყველა პროდუქტი</h2>
        ...
        {products.length === 0 ? (
          <p className="text-[#cbb3aa] text-sm py-20 text-center">პროდუქტები ჯერ არ არის.</p>
        ) : (
