      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 pb-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="flex flex-col">
            ...
            <button
              ...
              className="w-full py-3.5 rounded-full text-sm font-semibold shadow-sm mb-8 disabled:opacity-40 disabled:cursor-not-allowed theme-cta-btn"
            ...
            {deliveryEstimateText && (
              <p className="text-xs text-[#cbb3aa] mb-4">{deliveryEstimateText}</p>
            )}

            {tokens.trustBadges.length > 0 && (
              <div className="flex flex-wrap gap-x-4 gap-y-1 mb-8">
                {tokens.trustBadges.map((badge, i) => (
                  <span key={i} className="text-xs text-[#cbb3aa]">✓ {getTrustBadgeText(badge, lang)}</span>
                ))}
              </div>
            )}
            ...
            {product.videoUrl && (
              <div className="pt-6 border-t border-[#f3e3de]">
                <p className="text-xs font-semibold uppercase tracking-widest text-[#cbb3aa] mb-3">{t.product.video}</p>
        ...
        {tokens.showRelatedProducts && product.relatedProducts.length > 0 && (
          <div className="mt-14 pt-8 border-t border-[#f3e3de]">
            <h2 className="font-bold text-2xl text-[#3d2b28] mb-6">{relatedProductsHeading || t.product.relatedProducts}</h2>
