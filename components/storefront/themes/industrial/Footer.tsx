...
export function Footer({
  ...
  const contactFormSection = tokens.footerContactForm !== 'off' && (
    <div className="border-t border-b border-[#3a3a38]">
  ...
  return (
    <footer className="bg-[#242422] border-t border-[#3a3a38] mt-auto">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 flex flex-col gap-4">
        ...
        {hasContactInfo && (
          <div className="flex flex-wrap items-center gap-x-6 gap-y-1.5 pb-4 border-b border-[#3a3a38] text-xs text-[#8a8a86]">
            {tokens.contactEmail && <a href={`mailto:${tokens.contactEmail}`} className="hover:text-[#f2f2f0] transition-colors">{tokens.contactEmail}</a>}
            {tokens.contactPhone && <a href={`tel:${tokens.contactPhone}`} className="hover:text-[#f2f2f0] transition-colors">{tokens.contactPhone}</a>}
            {tokens.contactAddress && <span>{tokens.contactAddress}</span>}
          </div>
        )}

        {pages.length > 0 && (
          <div className="flex flex-wrap items-center gap-x-6 gap-y-1.5 text-xs text-[#8a8a86]">
            {pages.map(page => (
              <Link key={page.id} href={`/pages/${page.slug}`} className="hover:text-[#f2f2f0] transition-colors">
        ...
        {tokens.footerLinkColumns.length > 0 && (
          <div className="flex flex-wrap gap-x-10 gap-y-4 pb-4 border-b border-[#3a3a38]">
            {tokens.footerLinkColumns.map((column, i) => (
              <div key={i} className="flex flex-col gap-1.5 min-w-[100px]">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-[#6f6f6b]">{resolveThemeText(column.title, column.translations?.en, column.translations?.ru, lang)}</p>
                {column.links.map((link, j) => (
                  <a key={j} href={link.url} className="text-xs text-[#8a8a86] hover:text-[#f2f2f0] transition-colors">{resolveThemeText(link.label, link.translations?.en, link.translations?.ru, lang)}</a>
                ))}
              </div>
            ))}
          </div>
        ...
        {paymentLabels.length > 0 && (
          <div className="flex flex-wrap items-center gap-2">
            {paymentLabels.map(label => (
              <span key={label} className="border border-[#3a3a38] px-2.5 py-1 text-[10px] text-[#8a8a86]">{label}</span>
            ))}
          </div>
        )}

        <div className="flex flex-col sm:flex-row items-center justify-between gap-2.5">
          <p className="text-[#6f6f6b] text-xs">{getThemeText(tokens, 'footerCopyrightText', lang) || `© ${new Date().getFullYear()} ${storeName}`}</p>
          <div className="flex items-center gap-4">
            {showSocials && <SocialLinks tokens={tokens} />}
            <Link href={`/contact`} className="text-[#6f6f6b] text-xs hover:text-[#f2f2f0] transition-colors underline underline-offset-2">
              {contactLabel}
            </Link>
            {tokens.showPlatformAttribution && <p className="text-[#6f6f6b] text-xs">{t.footer.poweredBy}</p>}
          </div>
        </div>
      </div>
