    ...
    <div className="bg-[#1c1c1c] min-h-screen">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 pb-20">
        <Breadcrumbs
          ...
          t={t}
          textClassName="text-[#f2f2f0]"
          mutedClassName="text-[#6f6f6b] hover:text-[#f2f2f0]"
        />
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="flex gap-3">
              ...
              <div className="flex flex-col gap-2 shrink-0">
                {images.map((image, i) => (
                  <button
                    ...
                    className="w-16 h-16 overflow-hidden border-2 transition-colors bg-[#242422]"
                    style={{ borderColor: activeImage === i ? tokens.accentColor : '#3a3a38' }}
                  >
            ...
            <div className="relative flex-1 aspect-square border border-[#3a3a38] bg-[#242422] overflow-hidden">
          ...
          <div className="flex flex-col">
            <h1 className="font-black text-3xl sm:text-4xl text-[#f2f2f0] tracking-tight leading-tight mb-4">
              {productName}
            </h1>
            ...
            {salePrice !== null ? (
              <div className="flex items-center gap-3 mb-6">
                ...
                <span className="text-[#6f6f6b] text-lg line-through">₾{price.toFixed(2)}</span>
                <span className="bg-red-600 text-white text-[10px] font-bold uppercase tracking-wide px-2 py-1">
                  {t.product.saleBadge}
                </span>
              </div>
            ) : (
              <p className="font-black text-3xl text-[#f2f2f0] mb-6">
                ₾{price.toFixed(2)}
              </p>
            )}
            ...
            {sizeGuideContent && (
              <button
                ...
                className="text-xs text-[#6f6f6b] hover:text-[#f2f2f0] underline underline-offset-2 mb-4 text-left w-fit"
              >
                {t.product.sizeGuide}
              </button>
            )}
            ...
            <div className="flex items-center gap-3 mb-4">
              <div className="flex items-center border border-[#3a3a38] bg-[#242422]">
                <button
                  type="button"
                  onClick={() => setQuantity(q => Math.max(1, q - 1))}
                  className="w-9 h-10 flex items-center justify-center text-[#b5b5b0] hover:text-[#f2f2f0] transition-colors text-lg"
                >
                  −
                </button>
                <span className="w-10 text-center text-[#f2f2f0] text-sm font-semibold tabular-nums">{quantity}</span>
                <button
                  type="button"
              ...
              {selectionComplete && stock !== null && (
                <span className={`text-xs ${isLowStock ? 'text-red-500 font-medium' : 'text-[#6f6f6b]'}`}>
            ...
            <button
              ...
              className="w-full py-3.5 text-sm font-semibold shadow-sm mb-8 disabled:opacity-40 disabled:cursor-not-allowed theme-cta-btn"
            ...
            {deliveryEstimateText && (
              <p className="text-xs text-[#6f6f6b] mb-4">{deliveryEstimateText}</p>
            )}

            {tokens.trustBadges.length > 0 && (
              <div className="flex flex-wrap gap-x-4 gap-y-1 mb-8">
                {tokens.trustBadges.map((badge, i) => (
                  <span key={i} className="text-xs text-[#6f6f6b]">✓ {getTrustBadgeText(badge, lang)}</span>
                ))}
              </div>
            )}
            ...
            {productDescription && (
              <div className="border border-[#3a3a38] bg-[#242422] p-4">
                <p className="text-xs font-semibold uppercase tracking-widest text-[#6f6f6b] mb-3">{t.product.description}</p>
                <p className="text-[#b5b5b0] text-sm leading-relaxed">{productDescription}</p>
              </div>
            )}

            {product.videoUrl && (
              <div className="border border-[#3a3a38] bg-[#242422] p-4">
                <p className="text-xs font-semibold uppercase tracking-widest text-[#6f6f6b] mb-3">{t.product.video}</p>
        ...
        {tokens.showRelatedProducts && product.relatedProducts.length > 0 && (
          <div className="mt-14 pt-8 border-t border-[#3a3a38]">
            <h2 className="font-black text-2xl text-[#f2f2f0] tracking-tight mb-6">{relatedProductsHeading || t.product.relatedProducts}</h2>
