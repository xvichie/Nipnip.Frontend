      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="flex gap-3">
            ...
            <div className="relative flex-1 aspect-square border border-[#3a3a38] bg-[#242422] overflow-hidden">
              ...
              {images[activeImage] ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-[#54544e] text-xs">სურათი არ არის</div>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col">
            ...
            <button
              ...
              className="w-full py-3.5 text-sm font-semibold shadow-sm hover:shadow-md transition-shadow mb-8 disabled:opacity-40 disabled:cursor-not-allowed text-white"
            ...
            {tokens.deliveryEstimateText && (
              <p className="text-xs text-[#6f6f6b] mb-4">{tokens.deliveryEstimateText}</p>
            )}

            {tokens.trustBadges.length > 0 && (
              <div className="flex flex-wrap gap-x-4 gap-y-1 mb-8">
                {tokens.trustBadges.map((badge, i) => (
                  <span key={i} className="text-xs text-[#6f6f6b]">✓ {badge}</span>
                ))}
              </div>
            )}
            ...
            {product.description && (
              <div className="border border-[#3a3a38] bg-[#242422] p-4">
                <p className="text-xs font-semibold uppercase tracking-widest text-[#6f6f6b] mb-3">აღწერა</p>
                <p className="text-[#b5b5b0] text-sm leading-relaxed">{product.description}</p>
              </div>
            )}

            {product.videoUrl && (
              <div className="border border-[#3a3a38] bg-[#242422] p-4">
                <p className="text-xs font-semibold uppercase tracking-widest text-[#6f6f6b] mb-3">ვიდეო</p>
        ...
        {tokens.showRelatedProducts && product.relatedProducts.length > 0 && (
          <div className="mt-14 pt-8 border-t border-[#3a3a38]">
            <h2 className="font-black text-2xl text-[#f2f2f0] tracking-tight mb-6">{tokens.relatedProductsHeading || 'მსგავსი პროდუქტები'}</h2>
