...
export function ProductDetail({
  ...
  const lowStockText = isLowStock
    ? (lowStockMessage ? lowStockMessage.replace('{n}', String(stock)) : t.product.lowStock(stock!))
    : null
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 pb-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 xl:gap-16">
        ...
        <div className="flex flex-col">
          ...
          {deliveryEstimateText && (
            <p className="text-xs text-[#999] mb-4">{deliveryEstimateText}</p>
          )}

          {tokens.trustBadges.length > 0 && (
            <div className="flex flex-wrap gap-x-4 gap-y-1 mb-8">
              {tokens.trustBadges.map((badge, i) => (
                <span key={i} className="text-xs text-[#999]">✓ {getTrustBadgeText(badge, lang)}</span>
              ))}
            </div>
          )}
          ...
          {productDescription && (
            <div className="pt-8 border-t border-[#e5e5e5]">
              ...
              <p className="text-[#555] text-sm leading-relaxed">{productDescription}</p>
            </div>
          )}
      ...
      {tokens.showRelatedProducts && product.relatedProducts.length > 0 && (
        <div className="mt-16 pt-10 border-t border-[#e5e5e5]">
          <h2 className="font-black text-2xl text-[#111] tracking-tight mb-6">{relatedProductsHeading || t.product.relatedProducts}</h2>
