  ...
  return (
    <footer className="bg-[#111] text-white mt-auto">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 flex flex-col gap-6">
        ...
        {hasContactInfo && (
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 pb-6 border-b border-white/10 text-xs text-[#999]">
        ...
        {pages.length > 0 && (
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-[#999]">
            {pages.map(page => (
          ...
          <div className="flex flex-wrap gap-x-10 gap-y-6 pb-6 border-b border-white/10">
            {tokens.footerLinkColumns.map((column, i) => (
              <div key={i} className="flex flex-col gap-2 min-w-[100px]">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-[#999]">{resolveThemeText(column.title, column.translations?.en, column.translations?.ru, lang)}</p>
                {column.links.map((link, j) => (
                  <a key={j} href={link.url} className="text-xs text-[#999] hover:text-white transition-colors">{resolveThemeText(link.label, link.translations?.en, link.translations?.ru, lang)}</a>
                ))}
              </div>
            ))}
          </div>
        ...
        {paymentLabels.length > 0 && (
          <div className="flex flex-wrap items-center gap-2">
            {paymentLabels.map(label => (
              <span key={label} className="rounded-full border border-[#e5e5e5]/20 px-3 py-1 text-[10px] text-[#999]">{label}</span>
            ))}
          </div>
        )}

        <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-[#666] text-xs">{getThemeText(tokens, 'footerCopyrightText', lang) || `© ${new Date().getFullYear()} ${storeName}`}</p>
          <div className="flex items-center gap-4">
            {showSocials && <SocialLinks tokens={tokens} />}
            <Link href={`/contact`} className="text-[#666] text-xs hover:text-white transition-colors underline underline-offset-2">
              {contactLabel}
            </Link>
            {tokens.showPlatformAttribution && <p className="text-[#666] text-xs">{t.footer.poweredBy}</p>}
          </div>
        </div>
      </div>
