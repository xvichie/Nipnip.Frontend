    ...
    <footer className="bg-[#111] text-white mt-auto">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 flex flex-col gap-6">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-[#666] text-xs">© {new Date().getFullYear()} {storeName}</p>
          <div className="flex items-center gap-4">
            {showSocials && <SocialLinks tokens={tokens} />}
            <Link href={`/store/${slug}/contact`} className="text-[#666] text-xs hover:text-white transition-colors underline underline-offset-2">
              {tokens.contactLabel}
            </Link>
            <p className="text-[#666] text-xs">შექმნილია NipNip-ის მიერ</p>
          </div>
        </div>
      </div>
