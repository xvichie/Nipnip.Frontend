      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 pb-16">
          ...
          <div className="flex gap-3">
            ...
            <div className="flex-1 aspect-[4/5] bg-[#f2f2f2] overflow-hidden">
              {images[activeImage] ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-[#767676] text-xs">{t.product.noImage}</div>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col">
            ...
            {sizeGuideContent && (
              <button
                ...
                className="italic font-serif text-sm text-[#767676] hover:text-[#111111] underline underline-offset-2 mb-5 text-left w-fit"
              >
                {t.product.sizeGuide}
              </button>
            )}
            ...
            <div className="flex items-center gap-4 mb-5">
              ...
              {selectionComplete && stock !== null && (
                <span className={`text-xs ${isLowStock ? 'text-red-600 font-medium' : 'text-[#767676]'}`}>
            ...
            {deliveryEstimateText && (
              <p className="text-xs text-[#767676] mb-4">{deliveryEstimateText}</p>
            )}

            {tokens.trustBadges.length > 0 && (
              <div className="flex flex-wrap gap-x-4 gap-y-1 mb-8">
                {tokens.trustBadges.map((badge, i) => (
                  <span key={i} className="text-xs text-[#767676]">✓ {getTrustBadgeText(badge, lang)}</span>
                ))}
              </div>
            )}
            ...
            {productDescription && (
              <div className="pt-6 border-t border-black/10">
                <p className="italic font-serif text-sm text-[#767676] mb-3">{t.product.description}</p>
                <p className="font-serif text-base leading-relaxed max-w-md text-[#111111]">{productDescription}</p>
              </div>
            )}

            {product.videoUrl && (
              <div className="pt-6 border-t border-black/10">
                <p className="italic font-serif text-sm text-[#767676] mb-3">{t.product.video}</p>
        ...
        {tokens.showRelatedProducts && product.relatedProducts.length > 0 && (
          <div className="mt-16 pt-10 border-t border-black/10">
            <p className="italic font-serif text-sm text-[#767676] mb-6">{relatedProductsHeading || t.product.relatedProducts}</p>
