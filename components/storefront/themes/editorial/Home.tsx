  ...
  function buildHero(cfg: Required<ThemeConfig>) {
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {heroEyebrow && (
          <p className={`italic font-serif ${HERO_EYEBROW_SIZE_CLASS[cfg.heroEyebrowSize]} ${getHeroTextColorClass(cfg, 'eyebrow', 'text-[#767676]')} mb-4`}>{heroEyebrow}</p>
        )}
        ...
        {heroSubheadline && (
          <p className={`${getHeroTextColorClass(cfg, 'subheadline', 'text-[#767676]')} ${HERO_SUBHEADLINE_SIZE_CLASS[cfg.heroSubheadlineSize]} mb-9`}>{heroSubheadline}</p>
        )}
  ...
  return (
    <div className="bg-white min-h-screen">
      {getHomeSectionOrder(tokens).map(key => {
        ...
        return (
          <section key={key} className="border-t border-black/10" style={{ backgroundColor: section.backgroundColor || undefined }}>
            <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
              <CustomSectionBlock
                section={section}
                lang={lang}
