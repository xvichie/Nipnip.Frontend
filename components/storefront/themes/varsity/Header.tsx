      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-6">
          ...
          {tokens.logoUrl ? (
            <>
              ...
              {tokens.showStoreName && (
                <span className="font-black text-[#1d3557] text-lg uppercase tracking-tight truncate">{storeName}</span>
              )}
            </>
          ) : (
            <span className="font-black text-[#1d3557] text-lg uppercase tracking-tight truncate">{storeName}</span>
        ...
        {hasNav && (
          <nav className="hidden md:flex items-center gap-1.5">
            <Link
              href={`/products`}
              className="px-4 py-2 text-sm font-bold uppercase tracking-wide text-[#1d3557]/70 hover:text-[#1d3557] hover:bg-[#eef2f6] transition-colors whitespace-nowrap"
            >
              ყველა პროდუქტი
            </Link>
              ...
              isDropdown ? (
                <div className="relative" onMouseEnter={openCategoryMenu} onMouseLeave={closeCategoryMenuDelayed}>
                  <button
                    ...
                    className="flex items-center gap-1 px-4 py-2 text-sm font-bold uppercase tracking-wide text-[#1d3557]/70 hover:text-[#1d3557] hover:bg-[#eef2f6] transition-colors whitespace-nowrap"
                  >
                    კატეგორიები
                  ...
                  {categoriesOpen && (
                    <div className="absolute top-full left-0 mt-2 min-w-[190px] bg-white shadow-xl border border-[#e3e7ec] p-2 flex flex-col gap-0.5 z-10">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          {!isCheckout && (
            <Link
              href={`/cart`}
              className="relative flex items-center gap-2 px-5 py-2.5 text-sm font-black uppercase tracking-wide text-white transition-transform hover:scale-105"
            ...
            >
              ...
              {count > 0 && (
                <span className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-[#1d3557] text-white text-[10px] font-black flex items-center justify-center">
                  {count > 9 ? '9+' : count}
                </span>
              )}
            </Link>
          ...
          {hasNav && (
            <button
              ...
              aria-label="მენიუ"
              className="md:hidden flex items-center justify-center w-9 h-9 text-[#1d3557]/70 hover:text-[#1d3557] hover:bg-[#eef2f6] transition-colors"
            >
        ...
        <div className="md:hidden fixed inset-0 z-50">
          ...
          <div className="absolute top-0 right-0 h-full w-72 max-w-[80vw] bg-white shadow-xl flex flex-col p-5 overflow-y-auto">
            <button
              ...
              aria-label="დახურვა"
              className="self-end mb-3 w-8 h-8 flex items-center justify-center text-[#1d3557]/60 hover:bg-[#eef2f6]"
            >
            ...
            <Link
              href={`/products`}
              onClick={() => setMobileOpen(false)}
              className="px-3 py-2.5 text-sm font-black uppercase tracking-wide text-[#1d3557] hover:bg-[#eef2f6]"
            >
              ყველა პროდუქტი
            </Link>
            {navCategories.map(category => (
            ...
            {navPages.map(page => (
              <Link
                ...
                href={`/pages/${page.slug}`}
                onClick={() => setMobileOpen(false)}
                className="px-3 py-2.5 text-sm font-semibold uppercase tracking-wide text-[#1d3557]/70 hover:bg-[#eef2f6]"
              >
                {page.title}
              </Link>
            ))}
