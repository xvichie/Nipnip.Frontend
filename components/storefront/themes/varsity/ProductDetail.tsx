      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 xl:gap-16">
          ...
          <div className="flex flex-col">
            ...
            {deliveryEstimateText && (
              <p className="text-xs font-medium text-[#a8b3bd] mb-4">{deliveryEstimateText}</p>
            )}

            {tokens.trustBadges.length > 0 && (
              <div className="flex flex-wrap gap-x-4 gap-y-1 mb-8">
                {tokens.trustBadges.map((badge, i) => (
                  <span key={i} className="text-xs font-medium text-[#a8b3bd]">✓ {getTrustBadgeText(badge, lang)}</span>
                ))}
              </div>
            )}
            ...
            {productDescription && (
              <div className="pt-8 border-t border-[#e3e7ec]">
                <p className="text-xs font-black uppercase tracking-widest text-[#a8b3bd] mb-3">{t.product.description}</p>
                <p className="text-[#5a6b7a] text-sm leading-relaxed">{productDescription}</p>
              </div>
            )}

            {product.videoUrl && (
              <div className="pt-8 border-t border-[#e3e7ec]">
                <p className="text-xs font-black uppercase tracking-widest text-[#a8b3bd] mb-3">{t.product.video}</p>
        ...
        {tokens.showRelatedProducts && product.relatedProducts.length > 0 && (
          <div className="mt-16 pt-10 border-t border-[#e3e7ec]">
            <h2 className="font-black text-2xl text-[#1d3557] uppercase tracking-tight mb-6">{relatedProductsHeading || t.product.relatedProducts}</h2>
