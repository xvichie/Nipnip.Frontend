  ...
  function buildHero(t: Required<ThemeConfig>) {
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {t.heroEyebrow && (
          <p
            className={`inline-block ${HERO_EYEBROW_SIZE_CLASS[t.heroEyebrowSize]} font-black uppercase tracking-widest px-3 py-1 mb-4`}
            style={{ backgroundColor: `${t.accentColor}18`, color: t.accentColor }}
          >
            {t.heroEyebrow}
          </p>
        )}
        <h1 className={`font-black uppercase tracking-tight mb-5 leading-[1.05] ${getHeroTextColorClass(t, 'headline', 'text-[#1d3557]')} ${HERO_HEADLINE_SIZE_CLASS[t.heroHeadlineSize]}`}>
          {t.heroHeadline || store.name}
        </h1>
        {t.heroSubheadline && (
          <p className={`${getHeroTextColorClass(t, 'subheadline', 'text-[#5a6b7a]')} ${HERO_SUBHEADLINE_SIZE_CLASS[t.heroSubheadlineSize]} mb-8`}>{t.heroSubheadline}</p>
        )}
          ...
          <div className={`flex flex-wrap items-center gap-3 ${getHeroButtonRowClass(t)}`}>
            {t.heroCtaEnabled && (
              <Link
                ...
                style={{ backgroundColor: t.accentColor, boxShadow: `6px 6px 0 #1d3557` }}
              >
    ...
    const heroImage = (
      <div className={`relative h-full ${imageOrderClass} ${heroImageHiddenMobile ? 'hidden md:block' : ''}`}>
        ...
        <div
          ...
          style={{ boxShadow: `10px 10px 0 ${t.accentColor}30` }}
        >
      ...
      <section className="relative overflow-hidden" style={sectionStyle}>
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {t.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={t.heroTextTheme === 'dark' ? 'text-[#1d3557]' : 'text-white'} />}
          </div>
        ) : isSplitHero ? (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className={`grid ${LANDING_CATEGORY_GRID_CLASS[tokens.landingCategoryColumns]} gap-3`}>
          {landingCategories.map((category, i) => {
            const palette = ['#1d3557', '#e8a92d']
            const color = palette[i % palette.length]
            return (
              <Link
                key={category.id}
                href={`/products/category/${category.slug}`}
                className="flex flex-col items-center justify-center gap-2.5 border border-[#e3e7ec] px-4 py-6 text-center transition-transform hover:scale-105"
              >
                ...
                <span className="text-sm font-black uppercase tracking-wide" style={{ color }}>{category.name}</span>
              </Link>
            )
          })}
        </div>
    ...
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14">
        <div className="flex items-end justify-between mb-8">
          <h2 className="font-black text-3xl text-[#1d3557] uppercase tracking-tight">ყველა პროდუქტი</h2>
          <Link
            href={`/products`}
            className="text-sm font-black uppercase tracking-wide transition-opacity hover:opacity-70"
        ...
        {products.length === 0 ? (
          <p className="text-[#7d8a9a] text-sm py-20 text-center">პროდუქტები ჯერ არ არის.</p>
        ) : (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14 flex flex-col gap-14">
        {landingCollections.map(collection => (
          <ProductScrollRow
            ...
            titleClassName="font-black text-3xl text-[#1d3557] uppercase tracking-tight"
            viewAllClassName="text-sm font-black uppercase tracking-wide transition-opacity hover:opacity-70"
            viewAllStyle={{ color: tokens.accentColor }}
            emptyMessageClassName="text-[#7d8a9a] text-sm py-16 text-center"
          />
        ))}
      </div>
    </section>
    ...
    <section key="content" style={getSectionBackgroundStyle(tokens, 'content')}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14">
        <ContentBlock
          ...
          buttonStyle={{ backgroundColor: tokens.accentColor, boxShadow: '6px 6px 0 #1d3557' }}
        />
      </div>
    </section>
  )
