          ...
          <div className={`flex flex-wrap items-center gap-3 ${getHeroButtonRowClass(cfg)}`}>
            {cfg.heroCtaEnabled && (
              <Link
                ...
                className="inline-flex items-center gap-2 text-sm font-black uppercase tracking-wide px-8 py-4 theme-cta-btn"
                style={{ backgroundColor: cfg.accentColor, color: cfg.buttonTextColor, boxShadow: `6px 6px 0 #1d3557`, '--btn-hover-bg': getButtonHoverColor(cfg) } as React.CSSProperties}
              >
      ...
      <section className="relative overflow-hidden" style={sectionStyle}>
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {cfg.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={cfg.heroTextTheme === 'dark' ? 'text-[#1d3557]' : 'text-white'} />}
          </div>
        ) : isSplitHero ? (
          ...
          {landingCategories.map((category, i) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-sm font-black uppercase tracking-wide" style={{ color }}>{getCategoryName(category, lang)}</span>
              </Link>
            )
          })}
        </div>
    ...
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14">
        <div className="flex items-end justify-between mb-8">
          <h2 className="font-black text-3xl text-[#1d3557] uppercase tracking-tight">{t.home.allProductsHeading}</h2>
          <Link
            href={`/products`}
        ...
        {products.length === 0 ? (
          <p className="text-[#7d8a9a] text-sm py-20 text-center">{t.home.noProductsYet}</p>
        ) : (
  ...
  return (
    <div className="bg-white min-h-screen">
      {getHomeSectionOrder(tokens).map(key => {
        ...
        return (
          <section key={key} style={{ backgroundColor: section.backgroundColor || undefined }}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14">
              <CustomSectionBlock
                section={section}
                lang={lang}
