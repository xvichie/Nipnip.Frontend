    ...
    <footer className="bg-white border-t border-[#ffe9d2] mt-auto">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 flex flex-col gap-6">
          ...
          <div className="flex flex-wrap gap-x-10 gap-y-6 pb-6 border-b border-[#ffe9d2]">
            {tokens.footerLinkColumns.map((column, i) => (
              <div key={i} className="flex flex-col gap-2 min-w-[100px]">
                <p className="text-[10px] font-bold uppercase tracking-widest text-[#9a9a9a]">{column.title}</p>
                {column.links.map((link, j) => (
                  <a key={j} href={link.url} className="text-sm text-[#9a9a9a] hover:text-[#2b2b2b] transition-colors">{link.label}</a>
                ))}
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-[#9a9a9a] text-sm font-medium">{tokens.footerCopyrightText || `© ${new Date().getFullYear()} ${storeName}`}</p>
          <div className="flex items-center gap-4">
            ...
            {tokens.showPlatformAttribution && <p className="text-[#9a9a9a] text-sm font-medium">შექმნილია NipNip-ის მიერ</p>}
          </div>
        </div>
      </div>
