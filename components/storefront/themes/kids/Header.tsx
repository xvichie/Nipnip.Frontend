          ...
          {tokens.logoUrl ? (
            <>
              ...
              {tokens.showStoreName && (
                <span className="font-black text-[#2b2b2b] text-lg tracking-tight truncate">{storeName}</span>
              )}
            </>
          ) : (
            <span className="font-black text-[#2b2b2b] text-lg tracking-tight truncate">{storeName}</span>
              ...
              isDropdown ? (
                <div className="relative" onMouseEnter={openCategoryMenu} onMouseLeave={closeCategoryMenuDelayed}>
                  <button
                    ...
                    className="flex items-center gap-1 rounded-full px-4 py-2 text-sm font-semibold text-[#2b2b2b]/70 hover:text-[#2b2b2b] hover:bg-[#eaf7fc] transition-colors whitespace-nowrap"
                  >
                    {t.header.categories}
                  ...
                  {categoriesOpen && (
                    <div className="absolute top-full left-0 mt-2 min-w-[190px] rounded-2xl bg-white shadow-xl border border-[#ffe9d2] p-2 flex flex-col gap-0.5 z-10">
                      {navCategories.map(category => (
                        <Link
                          key={category.id}
                          href={`/products/category/${category.slug}`}
                          onClick={() => setCategoriesOpen(false)}
                          className={`rounded-xl py-2 text-sm font-medium text-[#2b2b2b]/70 hover:text-[#2b2b2b] hover:bg-[#eaf7fc] transition-colors whitespace-nowrap ${category.isChild ? 'pl-7 pr-3.5' : 'px-3.5'}`}
                        >
                ...
                navCategories.map(category => (
                  <Link
                    key={category.id}
                    href={`/products/category/${category.slug}`}
                    className={`rounded-full px-4 py-2 text-sm font-semibold text-[#2b2b2b]/70 hover:text-[#2b2b2b] hover:bg-[#eaf7fc] transition-colors whitespace-nowrap ${category.isChild ? 'pl-3' : ''}`}
                  >
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          {hasNav && (
            <button
              ...
              className="md:hidden flex items-center justify-center w-9 h-9 rounded-full text-[#2b2b2b]/70 hover:text-[#2b2b2b] hover:bg-[#eaf7fc] transition-colors"
            >
        ...
        <div className="md:hidden fixed inset-0 z-50">
          ...
          <div className="absolute top-0 right-0 h-full w-72 max-w-[80vw] bg-white shadow-xl flex flex-col p-5 overflow-y-auto rounded-l-3xl">
            <button
              ...
              className="self-end mb-3 w-8 h-8 flex items-center justify-center rounded-full text-[#2b2b2b]/60 hover:bg-[#eaf7fc]"
            >
            ...
            <Link
              href={`/products`}
              onClick={() => setMobileOpen(false)}
              className="rounded-xl px-3 py-2.5 text-sm font-bold text-[#2b2b2b] hover:bg-[#eaf7fc]"
            >
              {t.header.allProducts}
            </Link>
            {navCategories.map(category => (
              <Link
                ...
                href={`/products/category/${category.slug}`}
                onClick={() => setMobileOpen(false)}
                className={`rounded-xl py-2.5 text-sm font-medium text-[#2b2b2b]/70 hover:bg-[#eaf7fc] ${category.isChild ? 'pl-6' : 'px-3'}`}
              >
                {getCategoryName(category, lang)}
              </Link>
            ))}
