    ...
    <div className="bg-[#fffbea] min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 pb-20">
        <Breadcrumbs
          ...
          mutedClassName="text-[#9a9a9a] hover:text-[#2b2b2b]"
        />
          ...
          <div className="flex gap-3">
              ...
              <div className="flex flex-col gap-2 shrink-0">
                {images.map((image, i) => (
                  <button
                    ...
                    style={{ borderColor: activeImage === i ? tokens.accentColor : '#ffe9d2' }}
                  >
            ...
            >
              {images[activeImage] ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-[#b8b8b8] text-xs font-medium">{t.product.noImage}</div>
              )}
            </div>
          </div>

          <div className="flex flex-col">
            <h1 className="font-black text-4xl sm:text-5xl text-[#2b2b2b] tracking-tight leading-[1.05] mb-5">
              {productName}
            </h1>
            ...
            {salePrice !== null ? (
              <div className="flex items-center gap-3 mb-8">
                ...
                <span className="text-[#b8b8b8] text-xl line-through">₾{price.toFixed(2)}</span>
              </div>
            ) : (
            ...
            {sizeGuideContent && (
              <button
                ...
                className="text-xs font-medium text-[#9a9a9a] hover:text-[#2b2b2b] underline underline-offset-2 mb-5 text-left w-fit"
              >
                {t.product.sizeGuide}
              </button>
            )}
            ...
            <div className="flex items-center gap-3 mb-5">
              <div className="flex items-center rounded-full bg-white border border-[#ffe9d2]">
                <button
                  type="button"
                  onClick={() => setQuantity(q => Math.max(1, q - 1))}
                  className="w-10 h-11 flex items-center justify-center text-[#2b2b2b]/60 hover:text-[#2b2b2b] transition-colors text-lg"
                >
                  −
                </button>
                <span className="w-10 text-center text-[#2b2b2b] text-sm font-bold tabular-nums">{quantity}</span>
                <button
                  type="button"
              ...
              {selectionComplete && stock !== null && (
                <span className={`text-xs font-medium ${isLowStock ? 'text-red-600' : 'text-[#9a9a9a]'}`}>
            ...
            <button
              ...
              className="w-full py-4 rounded-full text-sm font-bold mb-8 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100 theme-cta-btn"
              style={{
                backgroundColor: added ? '#2d6a2d' : tokens.accentColor,
                color: tokens.buttonTextColor,
                boxShadow: added ? 'none' : `0 16px 40px -12px ${tokens.accentColor}88`,
            ...
            {deliveryEstimateText && (
              <p className="text-xs font-medium text-[#9a9a9a] mb-4">{deliveryEstimateText}</p>
            )}

            {tokens.trustBadges.length > 0 && (
              <div className="flex flex-wrap gap-x-4 gap-y-1 mb-8">
                {tokens.trustBadges.map((badge, i) => (
                  <span key={i} className="text-xs font-medium text-[#9a9a9a]">✓ {getTrustBadgeText(badge, lang)}</span>
                ))}
              </div>
            )}

            {productDescription && (
              <div className="pt-8 border-t border-[#ffe9d2]">
                <p className="text-xs font-bold uppercase tracking-widest text-[#b8b8b8] mb-3">{t.product.description}</p>
                <p className="text-[#6b6b6b] text-sm leading-relaxed">{productDescription}</p>
              </div>
            )}

            {product.videoUrl && (
              <div className="pt-8 border-t border-[#ffe9d2]">
                <p className="text-xs font-bold uppercase tracking-widest text-[#b8b8b8] mb-3">{t.product.video}</p>
        ...
        {tokens.showRelatedProducts && product.relatedProducts.length > 0 && (
          <div className="mt-16 pt-10 border-t border-[#ffe9d2]">
            <h2 className="font-black text-2xl text-[#2b2b2b] tracking-tight mb-6">{relatedProductsHeading || t.product.relatedProducts}</h2>
