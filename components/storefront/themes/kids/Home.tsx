  ...
  function buildHero(t: Required<ThemeConfig>) {
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        ...
        <h1 className={`font-black tracking-tight mb-5 leading-[1.05] ${getHeroTextColorClass(t, 'headline', 'text-[#2b2b2b]')} ${HERO_HEADLINE_SIZE_CLASS[t.heroHeadlineSize]}`}>
      ...
      <section className="relative overflow-hidden" style={sectionStyle}>
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {t.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={t.heroTextTheme === 'dark' ? 'text-[#2b2b2b]' : 'text-white'} />}
          </div>
        ) : isSplitHero ? (
    ...
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14">
        <div className="flex items-end justify-between mb-8">
          <h2 className="font-black text-3xl text-[#2b2b2b] tracking-tight">ყველა პროდუქტი</h2>
          <Link
            href={`/products`}
        ...
        {products.length === 0 ? (
          <p className="text-[#9a9a9a] text-sm py-20 text-center">პროდუქტები ჯერ არ არის.</p>
        ) : (
