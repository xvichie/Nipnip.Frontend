  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 pb-24">
      <div className="mb-8 pb-6 border-b border-[#e6e1d9]">
        <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#8c877e] mb-1">კოლექცია</p>
        <h1 className="font-black text-3xl text-[#1f1d1b] tracking-tight">{activeCategoryName ?? activeCollectionName ?? 'ყველა პროდუქტი'}</h1>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-10">
        ...
        <aside className="w-full md:w-48 shrink-0 flex flex-col gap-8 md:sticky md:top-14 md:self-start md:max-h-[calc(100vh-4.5rem)] md:overflow-y-auto md:pb-6">
          {displayCategories.length > 0 && (
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-widest text-[#8c877e] mb-3">კატეგორიები</p>
        ...
        <div className="flex-1">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 pb-4 border-b border-[#e6e1d9]">
            <p className="text-sm text-[#6b665e]">
              <span className="text-[#1f1d1b] font-semibold">{totalCount}</span> პროდუქტი
            </p>
          ...
          {isLoading ? (
            ...
            <div className="py-32 flex flex-col items-center gap-4 text-center">
              <p className="text-[#8c877e] text-sm">პროდუქტი ვერ მოიძებნა.</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4">
