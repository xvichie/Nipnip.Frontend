  ...
  function buildHero(cfg: Required<ThemeConfig>) {
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {cfg.heroEyebrow && (
          <p className={`${HERO_EYEBROW_SIZE_CLASS[cfg.heroEyebrowSize]} font-semibold uppercase tracking-[0.28em] ${getHeroTextColorClass(cfg, 'eyebrow', 'text-[#8c877e]')} mb-4`}>{cfg.heroEyebrow}</p>
        )}
        ...
        {cfg.heroSubheadline && (
          <p className={`${getHeroTextColorClass(cfg, 'subheadline', 'text-[#6b665e]')} ${HERO_SUBHEADLINE_SIZE_CLASS[cfg.heroSubheadlineSize]} mb-6`}>{cfg.heroSubheadline}</p>
        )}
