  ...
  function buildHero(cfg: Required<ThemeConfig>) {
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {heroEyebrow && (
          <p className={`${HERO_EYEBROW_SIZE_CLASS[cfg.heroEyebrowSize]} font-semibold uppercase tracking-[0.28em] ${getHeroTextColorClass(cfg, 'eyebrow', 'text-[#8c877e]')} mb-4`}>{heroEyebrow}</p>
        )}
        <h1 className={`font-black tracking-tight mb-4 ${getHeroTextColorClass(cfg, 'headline', 'text-[#1f1d1b]')} ${HERO_HEADLINE_SIZE_CLASS[cfg.heroHeadlineSize]}`}>
          {heroHeadline || store.name}
        </h1>
        {heroSubheadline && (
          <p className={`${getHeroTextColorClass(cfg, 'subheadline', 'text-[#6b665e]')} ${HERO_SUBHEADLINE_SIZE_CLASS[cfg.heroSubheadlineSize]} mb-6`}>{heroSubheadline}</p>
        )}
          ...
          <div className={`flex flex-wrap items-center gap-3 ${getHeroButtonRowClass(cfg)}`}>
            {cfg.heroCtaEnabled && (
              <Link
                ...
                className="inline-flex items-center gap-2 text-xs font-bold px-7 py-3.5 uppercase tracking-widest theme-cta-btn"
            ...
            {cfg.heroSecondaryCtaEnabled && (
              <Link
                ...
                className="inline-flex items-center gap-2 border text-xs font-bold px-7 py-3.5 uppercase tracking-widest theme-cta-btn"
    ...
    return (
      ...
      >
        {sectionBanner && cfg.bannerType === 'image' && <div className="absolute inset-0 bg-[#f6f4f1]/80" />}
        ...
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {cfg.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={cfg.heroTextTheme === 'dark' ? 'text-[#1f1d1b]' : 'text-white'} />}
          </div>
        ) : isSplitHero ? (
        ...
        <div className={`grid ${LANDING_CATEGORY_GRID_CLASS[tokens.landingCategoryColumns]} gap-3`}>
          {landingCategories.map(category => (
            ...
            >
              ...
              <span className="text-sm text-[#6b665e]">{getCategoryName(category, lang)}</span>
            </Link>
          ))}
        </div>
      </div>
    ...
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14">
        <div className="flex items-end justify-between mb-8">
          <h2 className="font-black text-2xl text-[#1f1d1b] tracking-tight">{t.home.allProductsHeading}</h2>
          <Link
            href={`/products`}
        ...
        {products.length === 0 ? (
          <p className="text-[#8c877e] text-sm py-20 text-center">{t.home.noProductsYet}</p>
        ) : (
  ...
  return (
    <div className="bg-[#f6f4f1]">
      {getHomeSectionOrder(tokens).map(key => {
        ...
        return (
          <section key={key} className="border-b border-[#e6e1d9]" style={{ backgroundColor: section.backgroundColor || undefined }}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14">
              <CustomSectionBlock
                section={section}
                lang={lang}
