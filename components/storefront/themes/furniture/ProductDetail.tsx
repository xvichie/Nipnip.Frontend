    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 pb-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 xl:gap-16">
        ...
        <div className="flex flex-col">
          ...
          {deliveryEstimateText && (
            <p className="text-xs text-[#8c877e] mb-4">{deliveryEstimateText}</p>
          )}

          {tokens.trustBadges.length > 0 && (
            <div className="flex flex-wrap gap-x-4 gap-y-1 mb-8">
              {tokens.trustBadges.map((badge, i) => (
                <span key={i} className="text-xs text-[#8c877e]">✓ {getTrustBadgeText(badge, lang)}</span>
              ))}
            </div>
          )}
          ...
          {productDescription && (
            <div className="pt-8 border-t border-[#e6e1d9]">
              ...
              <p className="text-[#6b665e] text-sm leading-relaxed">{productDescription}</p>
            </div>
          )}
      ...
      {tokens.showRelatedProducts && product.relatedProducts.length > 0 && (
        <div className="mt-16 pt-10 border-t border-[#e6e1d9]">
          <h2 className="font-black text-2xl text-[#1f1d1b] tracking-tight mb-6">{relatedProductsHeading || t.product.relatedProducts}</h2>
