...
export function Header({
  ...
  return (
    <header
      className={`${tokens.headerSticky ? 'sticky top-0' : ''} z-50 bg-[#f6f4f1] border-b border-[#e6e1d9]`}
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-6">
          ...
          {tokens.logoUrl ? (
            <>
              ...
              {tokens.showStoreName && (
                <span className="font-black text-[#1f1d1b] text-base tracking-tight uppercase truncate">{storeName}</span>
              )}
            </>
          ) : (
            <span className="font-black text-[#1f1d1b] text-base tracking-tight uppercase truncate">{storeName}</span>
            ...
            {navCategories.length > 0 && (
              isDropdown ? (
                ...
                >
                  ...
                  {categoriesOpen && (
                    <div className="absolute top-full left-0 mt-1 min-w-[180px] bg-[#f6f4f1] shadow-lg border border-[#e6e1d9] py-1.5 flex flex-col z-10">
                      {navCategories.map(category => (
                        <Link
                          key={category.id}
                          href={`/products/category/${category.slug}`}
                          onClick={() => setCategoriesOpen(false)}
                          className={`py-2 text-sm text-[#6b665e] hover:text-[#1f1d1b] hover:bg-[#efeae2] transition-colors whitespace-nowrap ${category.isChild ? 'pl-8 pr-4' : 'px-4'}`}
                        >
                ...
                navCategories.map(category => (
                  <Link
                    key={category.id}
                    href={`/products/category/${category.slug}`}
                    className={`text-sm text-[#6b665e] hover:text-[#1f1d1b] transition-colors whitespace-nowrap ${category.isChild ? 'pl-3' : ''}`}
                  >
        ...
        <div className="flex items-center gap-1 sm:gap-3 shrink-0">
          ...
          {!isCheckout && (
            <Link
              href={`/cart`}
              className="relative flex items-center gap-2 text-sm text-[#6b665e] hover:text-[#1f1d1b] transition-colors"
            >
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50">
          ...
          <div className="absolute top-0 right-0 h-full w-72 max-w-[80vw] bg-[#f6f4f1] shadow-xl flex flex-col p-5 overflow-y-auto">
            <button
              type="button"
            ...
            {navCategories.map(category => (
              <Link
                ...
                href={`/products/category/${category.slug}`}
                onClick={() => setMobileOpen(false)}
                className={`text-sm text-[#6b665e] py-2.5 border-b border-[#efeae2] ${category.isChild ? 'pl-4' : ''}`}
              >
                {getCategoryName(category, lang)}
              </Link>
            ))}
