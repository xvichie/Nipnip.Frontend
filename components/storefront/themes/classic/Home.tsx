...
export function Home({
  ...
  const heroText = (
    <div className={`w-full max-w-xl ${textAlignClass}`}>
      {tokens.heroEyebrow && (
        <p className={`${HERO_EYEBROW_SIZE_CLASS[tokens.heroEyebrowSize]} font-semibold uppercase tracking-widest ${getHeroTextColorClass(tokens, 'eyebrow', 'text-gray-400')} mb-3`}>{tokens.heroEyebrow}</p>
      )}
      <h1 className={`font-bold tracking-tight mb-5 ${getHeroTextColorClass(tokens, 'headline', 'text-gray-900')} ${HERO_HEADLINE_SIZE_CLASS[tokens.heroHeadlineSize]}`}>
        {tokens.heroHeadline || store.name}
      </h1>
      {tokens.heroSubheadline && (
        <p className={`${getHeroTextColorClass(tokens, 'subheadline', 'text-gray-500')} ${HERO_SUBHEADLINE_SIZE_CLASS[tokens.heroSubheadlineSize]} mb-8`}>{tokens.heroSubheadline}</p>
      )}
