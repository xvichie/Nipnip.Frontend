...
export function Home({
  ...
  function buildHero(cfg: Required<ThemeConfig>) {
    ...
    const sectionStyle = sectionBanner
      ? cfg.bannerType === 'image'
        ? {
            backgroundImage: `linear-gradient(rgba(255,255,255,0.75),rgba(255,255,255,0.9)), url(${cfg.bannerUrl})`,
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {heroEyebrow && (
          <p className={`${HERO_EYEBROW_SIZE_CLASS[cfg.heroEyebrowSize]} font-semibold uppercase tracking-widest ${getHeroTextColorClass(cfg, 'eyebrow', 'text-gray-400')} mb-3`}>{heroEyebrow}</p>
        )}
        <h1 className={`font-bold tracking-tight mb-5 ${getHeroTextColorClass(cfg, 'headline', 'text-gray-900')} ${HERO_HEADLINE_SIZE_CLASS[cfg.heroHeadlineSize]}`}>
          {heroHeadline || store.name}
        </h1>
        {heroSubheadline && (
          <p className={`${getHeroTextColorClass(cfg, 'subheadline', 'text-gray-500')} ${HERO_SUBHEADLINE_SIZE_CLASS[cfg.heroSubheadlineSize]} mb-8`}>{heroSubheadline}</p>
        )}
      ...
      <section className="bg-white relative overflow-hidden" style={sectionStyle}>
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {cfg.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={cfg.heroTextTheme === 'dark' ? 'text-gray-900' : 'text-white'} />}
          </div>
        ) : isSplitHero ? (
  ...
  const categoriesSection = landingCategories.length > 0 && (
    <section key="categories" className="border-t border-gray-200" style={getSectionBackgroundStyle(tokens, 'categories')}>
    ...
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
        <div className="flex items-end justify-between mb-6">
          <h2 className="font-bold text-2xl text-gray-900">{t.home.allProductsHeading}</h2>
        ...
        {products.length === 0 ? (
          <p className="text-gray-400 text-sm py-20 text-center">{t.home.noProductsYet}</p>
        ) : (
  ...
  const collectionsSection = landingCollections.length > 0 && (
    <section key="collections" className="border-t border-gray-200" style={getSectionBackgroundStyle(tokens, 'collections')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14 flex flex-col gap-14">
        {landingCollections.map(collection => (
          <ProductScrollRow
            ...
            titleClassName="font-bold text-2xl text-gray-900"
            viewAllClassName="text-sm font-medium hover:underline"
            viewAllStyle={{ color: tokens.accentColor }}
            emptyMessageClassName="text-gray-400 text-sm py-16 text-center"
          />
        ))}
      </div>
    </section>
  ...
  const contentSection = hasContentBlock(tokens) && (
    <section key="content" className="border-t border-gray-200" style={getSectionBackgroundStyle(tokens, 'content')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
        <ContentBlock
          tokens={tokens}
          lang={lang}
          headingClassName="font-bold text-2xl sm:text-3xl text-gray-900 mb-4"
          bodyClassName="text-gray-500 text-sm sm:text-base leading-relaxed mb-6"
  ...
  const faqSection = tokens.showFaqSection && tokens.faqItems.length > 0 && (
    <section key="faq" className="border-t border-gray-200" style={getSectionBackgroundStyle(tokens, 'faq')}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-14">
        <FaqAccordion
          tokens={tokens}
          t={t}
          lang={lang}
          headingClassName="font-bold text-2xl sm:text-3xl text-gray-900 mb-6"
          questionClassName="font-medium text-sm text-gray-900"
          answerClassName="text-sm text-gray-500 leading-relaxed"
          borderClassName="divide-gray-200"
        />
      </div>
    </section>
  )
  ...
  return (
    <div className="bg-[#fafafa] min-h-screen">
      {getHomeSectionOrder(tokens).map(key => {
        ...
        return (
          <section key={key} className="border-t border-gray-200" style={{ backgroundColor: section.backgroundColor || undefined }}>
            <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
              <CustomSectionBlock
                section={section}
                lang={lang}
