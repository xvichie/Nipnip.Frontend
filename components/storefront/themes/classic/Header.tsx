          ...
          {tokens.logoUrl ? (
            <>
              ...
              {tokens.showStoreName && (
                <span className="font-bold text-gray-900 text-lg truncate">{storeName}</span>
              )}
            </>
          ) : (
            <span className="font-bold text-gray-900 text-lg truncate">{storeName}</span>
            ...
            {navCategories.length > 0 && (
              isDropdown ? (
                ...
                >
                  ...
                  {categoriesOpen && (
                    <div className="absolute top-full left-0 mt-1 min-w-[180px] rounded-md bg-white shadow-lg border border-gray-100 py-1.5 flex flex-col z-10">
                      {navCategories.map(category => (
                        <Link
                          key={category.id}
                          href={`/products/category/${category.slug}`}
                          onClick={() => setCategoriesOpen(false)}
                          className={`py-2 text-sm text-gray-600 hover:text-gray-900 hover:bg-gray-50 transition-colors whitespace-nowrap ${category.isChild ? 'pl-8 pr-4' : 'px-4'}`}
                        >
                ...
                navCategories.map(category => (
                  <Link
                    key={category.id}
                    href={`/products/category/${category.slug}`}
                    className={`text-sm text-gray-600 hover:text-gray-900 transition-colors whitespace-nowrap ${category.isChild ? 'pl-3' : ''}`}
                  >
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          {hasNav && (
            <button
              ...
              className="md:hidden flex items-center justify-center w-9 h-9 rounded-md text-gray-600 hover:text-gray-900 hover:bg-gray-100 transition-colors"
            >
        ...
        <div className="md:hidden fixed inset-0 z-50">
          ...
          <div className="absolute top-0 right-0 h-full w-72 max-w-[80vw] bg-white shadow-xl flex flex-col p-5 overflow-y-auto">
            <button
              ...
              className="self-end mb-3 w-8 h-8 flex items-center justify-center text-gray-600 hover:text-gray-900"
            >
            ...
            {navCategories.map(category => (
              <Link
                ...
                href={`/products/category/${category.slug}`}
                onClick={() => setMobileOpen(false)}
                className={`text-sm text-gray-600 py-2.5 border-b border-gray-100 ${category.isChild ? 'pl-4' : ''}`}
              >
                {getCategoryName(category, lang)}
              </Link>
            ))}
