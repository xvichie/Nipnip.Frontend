      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 pb-16">
          ...
          <div className="flex gap-3">
            ...
            <div className="flex-1 aspect-square rounded-md bg-white shadow-sm overflow-hidden">
              {images[activeImage] ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-gray-300 text-xs">{t.product.noImage}</div>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col">
            ...
            {sizeGuideContent && (
              <button
                ...
                className="text-xs text-gray-400 hover:text-gray-900 underline underline-offset-2 mb-4 text-left w-fit"
              >
                {t.product.sizeGuide}
              </button>
            )}
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              {selectionComplete && stock !== null && (
                <span className={`text-xs ${isLowStock ? 'text-red-600 font-medium' : 'text-gray-400'}`}>
            ...
            <button
              ...
              className="w-full py-3.5 rounded-md text-sm font-semibold shadow-sm mb-8 disabled:opacity-40 disabled:cursor-not-allowed theme-cta-btn"
            ...
            {deliveryEstimateText && (
              <p className="text-xs text-gray-400 mb-4">{deliveryEstimateText}</p>
            )}

            {tokens.trustBadges.length > 0 && (
              <div className="flex flex-wrap gap-x-4 gap-y-1 mb-8">
                {tokens.trustBadges.map((badge, i) => (
                  <span key={i} className="text-xs text-gray-400">✓ {getTrustBadgeText(badge, lang)}</span>
                ))}
              </div>
            )}
            ...
            {productDescription && (
              <div className="pt-6 border-t border-gray-200">
                <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-3">{t.product.description}</p>
            ...
            {product.videoUrl && (
              <div className="pt-6 border-t border-gray-200">
                <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-3">{t.product.video}</p>
        ...
        {tokens.showRelatedProducts && product.relatedProducts.length > 0 && (
          <div className="mt-14 pt-8 border-t border-gray-200">
            <h2 className="font-bold text-2xl text-gray-900 mb-6">{relatedProductsHeading || t.product.relatedProducts}</h2>
