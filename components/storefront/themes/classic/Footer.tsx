    ...
    <footer className="bg-white border-t border-gray-200 mt-auto">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 flex flex-col gap-5">
          ...
          <div className="flex flex-wrap gap-x-10 gap-y-6 pb-5 border-b border-gray-200">
            {tokens.footerLinkColumns.map((column, i) => (
              <div key={i} className="flex flex-col gap-2 min-w-[100px]">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-gray-400">{resolveThemeText(column.title, column.translations?.en, column.translations?.ru, lang)}</p>
                {column.links.map((link, j) => (
                  <a key={j} href={link.url} className="text-xs text-gray-500 hover:text-gray-900 transition-colors">{resolveThemeText(link.label, link.translations?.en, link.translations?.ru, lang)}</a>
                ))}
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-gray-400 text-xs">{getThemeText(tokens, 'footerCopyrightText', lang) || `© ${new Date().getFullYear()} ${storeName}`}</p>
          <div className="flex flex-wrap items-center justify-center gap-4">
            ...
            {tokens.showPlatformAttribution && <p className="text-gray-400 text-xs">{t.footer.poweredBy}</p>}
          </div>
        </div>
      </div>
