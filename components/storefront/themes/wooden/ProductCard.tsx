  ...
  return (
    <Link href={`/products/${product.slug}`} className="group block h-full">
      <div className="rounded-md bg-white shadow-sm hover:shadow-md transition-shadow duration-200 overflow-hidden h-full flex flex-col">
        <div className="relative aspect-square bg-[#f3ebe0] overflow-hidden">
          {badge && (
            <span
              className="absolute top-2 left-2 z-10 rounded-md text-white text-[10px] font-semibold uppercase tracking-wide px-2 py-1"
          ...
          {product.thumbnailUrl ? (
            ...
            <div className="w-full h-full flex items-center justify-center text-[#d4c8b8] text-xs">სურათი არ არის</div>
          )}
          <QuickAddButton
            slug={slug}
            productSlug={product.slug}
            className="absolute inset-x-0 bottom-0 z-10 w-full py-2.5 flex items-center justify-center gap-1.5 bg-[#4a3f35] text-white text-xs font-medium opacity-100 translate-y-0 [@media(hover:hover)]:opacity-0 [@media(hover:hover)]:translate-y-full [@media(hover:hover)]:group-hover:opacity-100 [@media(hover:hover)]:group-hover:translate-y-0 transition-all duration-200 hover:bg-[#3a3128] disabled:opacity-60"
          />
        </div>
        <div className="p-3.5 flex flex-col flex-1">
          {categoryName && <p className="text-[#c4b7a6] text-[11px] mb-1">{categoryName}</p>}
          <h3 className="text-[#4a3f35] font-medium text-sm leading-snug mb-2">{product.name}</h3>
          <div className="mt-auto">
            {product.salePrice !== null ? (
              <div className="flex items-center gap-2">
                ...
                <span className="text-[#c4b7a6] text-xs line-through">₾{product.basePrice.toFixed(2)}</span>
              </div>
            ) : (
              <span className="font-semibold text-sm" style={{ color: tokens.accentColor }}>
