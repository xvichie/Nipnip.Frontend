...
export function Home({
  ...
  function buildHero(t: Required<ThemeConfig>) {
    ...
    const sectionStyle = sectionBanner
      ? t.bannerType === 'image'
        ? {
            backgroundImage: `linear-gradient(rgba(250,246,239,0.75),rgba(250,246,239,0.9)), url(${t.bannerUrl})`,
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {t.heroEyebrow && (
          <p className={`${HERO_EYEBROW_SIZE_CLASS[t.heroEyebrowSize]} font-semibold uppercase tracking-widest ${getHeroTextColorClass(t, 'eyebrow', 'text-[#c4b7a6]')} mb-3`}>{t.heroEyebrow}</p>
        )}
        <h1 className={`font-bold tracking-tight mb-5 ${getHeroTextColorClass(t, 'headline', 'text-[#4a3f35]')} ${HERO_HEADLINE_SIZE_CLASS[t.heroHeadlineSize]}`}>
          {t.heroHeadline || store.name}
        </h1>
        {t.heroSubheadline && (
          <p className={`${getHeroTextColorClass(t, 'subheadline', 'text-[#8a7d6c]')} ${HERO_SUBHEADLINE_SIZE_CLASS[t.heroSubheadlineSize]} mb-8`}>{t.heroSubheadline}</p>
        )}
      ...
      <section className="bg-[#faf6ef] relative overflow-hidden" style={sectionStyle}>
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {t.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={t.heroTextTheme === 'dark' ? 'text-[#4a3f35]' : 'text-white'} />}
          </div>
        ) : isSplitHero ? (
    ...
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
        <div className="flex items-end justify-between mb-6">
          <h2 className="font-bold text-2xl text-[#4a3f35]">ყველა პროდუქტი</h2>
        ...
        {products.length === 0 ? (
          <p className="text-[#c4b7a6] text-sm py-20 text-center">პროდუქტები ჯერ არ არის.</p>
        ) : (
