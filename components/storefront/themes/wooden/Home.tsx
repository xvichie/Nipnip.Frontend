...
import { getBannerBackgroundStyle, getButtonHoverColor, getCustomSectionId, getHeroBackgroundImageClass, getHeroButtonRowClass, getHeroCtaHref, getHeroCtaLabel, getHeroImageClass, getHeroOverlayStyle, getHeroTextAlignClass, getHeroTextColorClass, getHeroSecondaryCtaHref, getHeroSecondaryCtaLabel, getHeroSlideTranslations, getHomeSectionOrder, getSectionBackgroundStyle, hasBanner, hasContentBlock, hasHeroVideo, HERO_EYEBROW_SIZE_CLASS, HERO_HEADLINE_SIZE_CLASS, HERO_HEIGHT_CLASS, HERO_SUBHEADLINE_SIZE_CLASS, HERO_TEXT_POSITION_CLASS, LANDING_CATEGORY_GRID_CLASS } from '@/lib/store/theme-config'
...
export function Home({
  ...
  function buildHero(cfg: Required<ThemeConfig>) {
    ...
    const sectionStyle = sectionBanner
      ? cfg.bannerType === 'image'
        ? {
            backgroundImage: `linear-gradient(rgba(250,246,239,0.75),rgba(250,246,239,0.9)), url(${cfg.bannerUrl})`,
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {heroEyebrow && (
          <p className={`${HERO_EYEBROW_SIZE_CLASS[cfg.heroEyebrowSize]} font-semibold uppercase tracking-widest ${getHeroTextColorClass(cfg, 'eyebrow', 'text-[#c4b7a6]')} mb-3`}>{heroEyebrow}</p>
        )}
        <h1 className={`font-bold tracking-tight mb-5 ${getHeroTextColorClass(cfg, 'headline', 'text-[#4a3f35]')} ${HERO_HEADLINE_SIZE_CLASS[cfg.heroHeadlineSize]}`}>
          {heroHeadline || store.name}
        </h1>
        {heroSubheadline && (
          <p className={`${getHeroTextColorClass(cfg, 'subheadline', 'text-[#8a7d6c]')} ${HERO_SUBHEADLINE_SIZE_CLASS[cfg.heroSubheadlineSize]} mb-8`}>{heroSubheadline}</p>
        )}
          ...
          <div className={`flex flex-wrap items-center gap-3 ${getHeroButtonRowClass(cfg)}`}>
            {cfg.heroCtaEnabled && (
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-md text-sm font-semibold px-7 py-3 theme-cta-btn"
            ...
            {cfg.heroSecondaryCtaEnabled && (
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-md border text-sm font-semibold px-7 py-3 theme-cta-btn"
    ...
    return (
      <section className="bg-[#faf6ef] relative overflow-hidden" style={sectionStyle}>
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {cfg.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={cfg.heroTextTheme === 'dark' ? 'text-[#4a3f35]' : 'text-white'} />}
          </div>
        ) : isSplitHero ? (
  ...
  const categoriesSection = landingCategories.length > 0 && (
    <section key="categories" className="border-t border-[#e8ddd0]" style={getSectionBackgroundStyle(tokens, 'categories')}>
        ...
        <div className={`grid ${LANDING_CATEGORY_GRID_CLASS[tokens.landingCategoryColumns]} gap-3`}>
          {landingCategories.map(category => (
            <Link
              ...
              className="flex flex-col items-center justify-center gap-2 rounded-md bg-white shadow-sm hover:shadow-md transition-shadow px-4 py-6 text-center text-sm font-medium text-[#8a7d6c]"
            >
    ...
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
        <div className="flex items-end justify-between mb-6">
          <h2 className="font-bold text-2xl text-[#4a3f35]">{t.home.allProductsHeading}</h2>
        ...
        {products.length === 0 ? (
          <p className="text-[#c4b7a6] text-sm py-20 text-center">{t.home.noProductsYet}</p>
        ) : (
  ...
  const collectionsSection = landingCollections.length > 0 && (
    <section key="collections" className="border-t border-[#e8ddd0]" style={getSectionBackgroundStyle(tokens, 'collections')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14 flex flex-col gap-14">
        {landingCollections.map(collection => (
          <ProductScrollRow
            ...
            titleClassName="font-bold text-2xl text-[#4a3f35]"
            viewAllClassName="text-sm font-medium hover:underline"
            viewAllStyle={{ color: tokens.accentColor }}
            emptyMessageClassName="text-[#c4b7a6] text-sm py-16 text-center"
          />
        ))}
      </div>
    </section>
  ...
  const contentSection = hasContentBlock(tokens) && (
    <section key="content" className="border-t border-[#e8ddd0]" style={getSectionBackgroundStyle(tokens, 'content')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
        <ContentBlock
          tokens={tokens}
          lang={lang}
          headingClassName="font-bold text-2xl sm:text-3xl text-[#4a3f35] mb-4"
          bodyClassName="text-[#8a7d6c] text-sm sm:text-base leading-relaxed mb-6"
  ...
  const faqSection = tokens.showFaqSection && tokens.faqItems.length > 0 && (
    <section key="faq" className="border-t border-[#e8ddd0]" style={getSectionBackgroundStyle(tokens, 'faq')}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-14">
        <FaqAccordion
          tokens={tokens}
          t={t}
          lang={lang}
          headingClassName="font-bold text-2xl sm:text-3xl text-[#4a3f35] mb-6"
          questionClassName="font-medium text-sm text-[#4a3f35]"
          answerClassName="text-sm text-[#8a7d6c] leading-relaxed"
          borderClassName="divide-[#e8ddd0]"
        />
      </div>
    </section>
  )
  ...
  return (
    <div className="bg-[#faf6ef] min-h-screen">
      {getHomeSectionOrder(tokens).map(key => {
        ...
        return (
          <section key={key} className="border-t border-[#e8ddd0]" style={{ backgroundColor: section.backgroundColor || undefined }}>
            <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
              <CustomSectionBlock
                section={section}
                lang={lang}
