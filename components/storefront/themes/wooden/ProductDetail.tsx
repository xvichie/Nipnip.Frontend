      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 pb-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="flex flex-col">
            ...
            {deliveryEstimateText && (
              <p className="text-xs text-[#c4b7a6] mb-4">{deliveryEstimateText}</p>
            )}

            {tokens.trustBadges.length > 0 && (
              <div className="flex flex-wrap gap-x-4 gap-y-1 mb-8">
                {tokens.trustBadges.map((badge, i) => (
                  <span key={i} className="text-xs text-[#c4b7a6]">✓ {getTrustBadgeText(badge, lang)}</span>
                ))}
              </div>
            )}
            ...
            {productDescription && (
              <div className="pt-6 border-t border-[#e8ddd0]">
                ...
                <p className="text-[#8a7d6c] text-sm leading-relaxed">{productDescription}</p>
              </div>
            )}
        ...
        {tokens.showRelatedProducts && product.relatedProducts.length > 0 && (
          <div className="mt-14 pt-8 border-t border-[#e8ddd0]">
            <h2 className="font-bold text-2xl text-[#4a3f35] mb-6">{relatedProductsHeading || t.product.relatedProducts}</h2>
