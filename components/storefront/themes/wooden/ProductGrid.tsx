  ...
  return (
    <div className="bg-[#faf6ef] min-h-screen">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 pb-24">
        <div className="mb-8">
          <p className="text-xs font-semibold uppercase tracking-widest text-[#c4b7a6] mb-1">{t.grid.collectionEyebrow}</p>
          <h1 className="font-bold text-3xl text-[#4a3f35]">{activeCategoryName ?? activeCollectionName ?? t.grid.allProductsTitle}</h1>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <aside className="w-full md:w-52 shrink-0 flex flex-col gap-4 md:sticky md:top-16 md:self-start md:max-h-[calc(100vh-5rem)] md:overflow-y-auto md:pb-6">
            {displayCategories.length > 0 && (
              <div className="rounded-md bg-white shadow-sm p-4 flex flex-col gap-1">
                <p className="text-xs font-semibold uppercase tracking-widest text-[#c4b7a6] mb-2">{t.header.categories}</p>
                <Link
                  href={`/products`}
                  className={[
                    'rounded px-3 py-1.5 text-sm transition-colors',
                    !activeCategorySlug ? 'bg-[#f3ebe0] text-[#4a3f35] font-medium' : 'text-[#8a7d6c] hover:bg-[#f8f2e9]',
                  ].join(' ')}
                >
                ...
                {displayCategories.map(category => (
                  <Link
                    ...
                    className={[
                      ...
                      activeCategorySlug === category.slug ? 'bg-[#f3ebe0] text-[#4a3f35] font-medium' : 'text-[#8a7d6c] hover:bg-[#f8f2e9]',
            ...
            {bounds && (
              <div className="rounded-md bg-white shadow-sm p-4">
                <PriceRangeFilter
                  ...
                  t={t}
                  trackColorClassName="bg-[#e8ddd0]"
                  labelClassName="text-[#c4b7a6]"
                  valueClassName="text-[#4a3f35]"
                />
              </div>
            )}

            {(facets ?? []).length > 0 && (
              <div className="rounded-md bg-white shadow-sm p-4 flex flex-col gap-4">
                <OptionFiltersPanel
                  facets={facets ?? []}
                  ...
                  accentColor={tokens.accentColor}
                  labelClassName="text-[#c4b7a6]"
                  chipClassName="border-[#e8ddd0] bg-white text-[#8a7d6c] hover:bg-[#f8f2e9]"
                />
              </div>
            )}
          </aside>
          ...
          <div className="flex-1">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
              <p className="text-sm text-[#a89a8c] flex items-center gap-2">
                <span className="text-[#4a3f35] font-medium">{totalCount}</span> {t.grid.unitProduct}
                {isRefetching && (
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                {tokens.searchBarLocation !== 'header' && (
                  <input
                    ...
                    className="w-full sm:w-56 rounded-md bg-white shadow-sm border border-[#e8ddd0] text-sm px-3 py-2 text-[#4a3f35] placeholder:text-[#c4b7a6] focus:outline-none transition-colors"
                  />
                )}
                <select
                  value={sortBy}
                  onChange={e => setSortBy(e.target.value as ProductSortOption)}
                  className="w-full sm:w-auto shrink-0 rounded-md bg-white shadow-sm border border-[#e8ddd0] text-sm px-3 py-2 text-[#4a3f35] focus:outline-none transition-colors"
                >
            ...
            {isLoading ? (
              ...
              <div className="py-32 flex flex-col items-center gap-4 text-center">
                <p className="text-[#c4b7a6] text-sm">{t.grid.noProductsFound}</p>
              </div>
            ) : (
              <>
                ...
                <Pagination
                  ...
                  onChange={setPage}
                  t={t}
                  buttonClassName="btn btn-sm rounded-md bg-white shadow-sm border border-[#e8ddd0] text-[#a89a8c] hover:text-[#4a3f35] disabled:opacity-30"
                  textClassName="text-[#c4b7a6]"
                />
              </>
            )}
          </div>
