  ...
  return (
    <div className="bg-[#faf6ef] min-h-screen">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 pb-24">
        <div className="mb-8">
          <p className="text-xs font-semibold uppercase tracking-widest text-[#c4b7a6] mb-1">კოლექცია</p>
          <h1 className="font-bold text-3xl text-[#4a3f35]">{activeCategoryName ?? activeCollectionName ?? 'ყველა პროდუქტი'}</h1>
        </div>
          ...
          <aside className="w-full md:w-52 shrink-0 flex flex-col gap-4 md:sticky md:top-16 md:self-start md:max-h-[calc(100vh-5rem)] md:overflow-y-auto md:pb-6">
            {displayCategories.length > 0 && (
              <div className="rounded-md bg-white shadow-sm p-4 flex flex-col gap-1">
                <p className="text-xs font-semibold uppercase tracking-widest text-[#c4b7a6] mb-2">კატეგორიები</p>
                <Link
                  href={`/products`}
                  className={[
          ...
          <div className="flex-1">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
              <p className="text-sm text-[#a89a8c]">
                <span className="text-[#4a3f35] font-medium">{totalCount}</span> პროდუქტი
              </p>
            ...
            {isLoading ? (
              ...
              <div className="py-32 flex flex-col items-center gap-4 text-center">
                <p className="text-[#c4b7a6] text-sm">პროდუქტი ვერ მოიძებნა.</p>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4">
