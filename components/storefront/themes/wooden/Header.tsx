...
export function Header({
  ...
  return (
    <header
      className={`${tokens.headerSticky ? 'sticky top-0' : ''} z-50 bg-white shadow-sm`}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-6">
          ...
          {tokens.logoUrl ? (
            <>
              ...
              {tokens.showStoreName && (
                <span className="font-bold text-[#4a3f35] text-lg truncate">{storeName}</span>
              )}
            </>
          ) : (
            <span className="font-bold text-[#4a3f35] text-lg truncate">{storeName}</span>
        ...
        {hasNav && (
          <nav className="hidden md:flex items-center gap-5">
            <Link href={`/products`} className="text-sm text-[#8a7d6c] hover:text-[#4a3f35] transition-colors whitespace-nowrap">
              ყველა პროდუქტი
            </Link>
              ...
              isDropdown ? (
                ...
                >
                  <button
                    ...
                    className="flex items-center gap-1 text-sm text-[#8a7d6c] hover:text-[#4a3f35] transition-colors whitespace-nowrap"
                  >
                    კატეგორიები
                  ...
                  {categoriesOpen && (
                    <div className="absolute top-full left-0 mt-1 min-w-[180px] rounded-md bg-white shadow-lg border border-[#f0e8dc] py-1.5 flex flex-col z-10">
                      {navCategories.map(category => (
                        <Link
                          key={category.id}
                          href={`/products/category/${category.slug}`}
                          onClick={() => setCategoriesOpen(false)}
                          className={`py-2 text-sm text-[#8a7d6c] hover:text-[#4a3f35] hover:bg-[#f8f2e9] transition-colors whitespace-nowrap ${category.isChild ? 'pl-8 pr-4' : 'px-4'}`}
                        >
                          {category.name}
                        </Link>
                      ))}
                ...
                navCategories.map(category => (
                  <Link
                    key={category.id}
                    href={`/products/category/${category.slug}`}
                    className={`text-sm text-[#8a7d6c] hover:text-[#4a3f35] transition-colors whitespace-nowrap ${category.isChild ? 'pl-3' : ''}`}
                  >
            ...
            {navPages.map(page => (
              <Link
                ...
                className="text-sm text-[#8a7d6c] hover:text-[#4a3f35] transition-colors whitespace-nowrap"
              >
                {page.title}
              </Link>
            ))}
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          {!isCheckout && (
            <Link
              href={`/cart`}
              className="relative flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium text-[#4a3f35] bg-[#f3ebe0] hover:bg-[#ecdfcd] transition-colors"
            >
          ...
          {hasNav && (
            <button
              ...
              aria-label="მენიუ"
              className="md:hidden flex items-center justify-center w-9 h-9 rounded-md text-[#8a7d6c] hover:text-[#4a3f35] hover:bg-[#f3ebe0] transition-colors"
            >
        ...
        <div className="md:hidden fixed inset-0 z-50">
          ...
          <div className="absolute top-0 right-0 h-full w-72 max-w-[80vw] bg-white shadow-xl flex flex-col p-5 overflow-y-auto">
            <button
              ...
              aria-label="დახურვა"
              className="self-end mb-3 w-8 h-8 flex items-center justify-center text-[#8a7d6c] hover:text-[#4a3f35]"
            >
            ...
            <Link
              href={`/products`}
              onClick={() => setMobileOpen(false)}
              className="text-sm font-medium text-[#4a3f35] py-2.5 border-b border-[#f0e8dc]"
            >
              ყველა პროდუქტი
            </Link>
            {navCategories.map(category => (
              <Link
                ...
                href={`/products/category/${category.slug}`}
                onClick={() => setMobileOpen(false)}
                className={`text-sm text-[#8a7d6c] py-2.5 border-b border-[#f0e8dc] ${category.isChild ? 'pl-4' : ''}`}
              >
                {category.name}
              </Link>
            ))}
            {navPages.map(page => (
              <Link
                ...
                href={`/pages/${page.slug}`}
                onClick={() => setMobileOpen(false)}
                className="text-sm text-[#8a7d6c] py-2.5 border-b border-[#f0e8dc]"
              >
                {page.title}
              </Link>
            ))}
