    ...
    <footer className="bg-white border-t border-[#e8e8e8] mt-auto">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 flex flex-col gap-4">
          ...
          <div className="flex flex-wrap gap-x-10 gap-y-4 pb-4 border-b border-[#e8e8e8]">
            {tokens.footerLinkColumns.map((column, i) => (
              <div key={i} className="flex flex-col gap-1.5 min-w-[100px]">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-[#8a8a8a]">{column.title}</p>
                {column.links.map((link, j) => (
                  <a key={j} href={link.url} className="text-xs text-[#8a8a8a] hover:text-[#0f0f0f] transition-colors">{link.label}</a>
                ))}
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2.5">
          <p className="text-[#8a8a8a] text-xs">{tokens.footerCopyrightText || `© ${new Date().getFullYear()} ${storeName}`}</p>
          <div className="flex items-center gap-4">
            ...
            {tokens.showPlatformAttribution && <p className="text-[#8a8a8a] text-xs">შექმნილია NipNip-ის მიერ</p>}
          </div>
        </div>
      </div>
