      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 pb-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="flex flex-col">
            ...
            <button
              ...
              className="w-full py-3.5 text-sm font-black uppercase tracking-wide shadow-sm mb-8 disabled:opacity-40 disabled:cursor-not-allowed theme-cta-btn"
            ...
            {deliveryEstimateText && (
              <p className="text-xs text-[#8a8a8a] mb-4">{deliveryEstimateText}</p>
            )}

            {tokens.trustBadges.length > 0 && (
              <div className="flex flex-wrap gap-x-4 gap-y-1 mb-8">
                {tokens.trustBadges.map((badge, i) => (
                  <span key={i} className="text-xs text-[#8a8a8a]">✓ {getTrustBadgeText(badge, lang)}</span>
                ))}
              </div>
            )}
            ...
            {productDescription && (
              <div className="border border-[#e8e8e8] bg-[#f7f7f7] p-4">
                ...
                <p className="text-[#4d4d4d] text-sm leading-relaxed">{productDescription}</p>
              </div>
            )}
        ...
        {tokens.showRelatedProducts && product.relatedProducts.length > 0 && (
          <div className="mt-14 pt-8 border-t border-[#e8e8e8]">
            <h2 className="font-black text-2xl text-[#0f0f0f] uppercase tracking-tight mb-6">{relatedProductsHeading || t.product.relatedProducts}</h2>
