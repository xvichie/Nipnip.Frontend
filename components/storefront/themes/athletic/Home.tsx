...
export function Home({
  ...
  function buildHero(cfg: Required<ThemeConfig>) {
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {cfg.heroEyebrow && (
          <p className={`flex items-center gap-2 ${HERO_EYEBROW_SIZE_CLASS[cfg.heroEyebrowSize]} font-black uppercase tracking-widest ${getHeroTextColorClass(cfg, 'eyebrow', 'text-[#6b6b6b]')} mb-3`}>
            <span className="inline-block w-1.5 h-1.5 shrink-0" style={{ backgroundColor: cfg.accentColor }} aria-hidden />
            {cfg.heroEyebrow}
          </p>
        )}
        <h1 className={`font-black uppercase tracking-tight mb-5 ${getHeroTextColorClass(cfg, 'headline', 'text-[#0f0f0f]')} ${HERO_HEADLINE_SIZE_CLASS[cfg.heroHeadlineSize]}`}>
          {cfg.heroHeadline || store.name}
        </h1>
        {cfg.heroSubheadline && (
          <p className={`${getHeroTextColorClass(cfg, 'subheadline', 'text-[#6b6b6b]')} ${HERO_SUBHEADLINE_SIZE_CLASS[cfg.heroSubheadlineSize]} mb-8`}>{cfg.heroSubheadline}</p>
        )}
          ...
          <div className={`flex flex-wrap items-center gap-3 ${getHeroButtonRowClass(cfg)}`}>
            {cfg.heroCtaEnabled && (
              <Link
                ...
                className="inline-flex items-center gap-2 text-white text-sm font-black uppercase tracking-wide px-7 py-3 transition-transform hover:scale-[1.02]"
            ...
            {cfg.heroSecondaryCtaEnabled && (
              <Link
                ...
                className="inline-flex items-center gap-2 border-2 text-sm font-black uppercase tracking-wide px-7 py-3 transition-colors"
    ...
    const heroImage = (
      <div className={`relative h-full ${imageOrderClass} ${heroImageHiddenMobile ? 'hidden md:block' : ''}`}>
        ...
        <div className={`relative h-full flex items-center justify-center overflow-hidden ${imageBanner || sectionBanner ? '' : 'bg-white border border-[#e8e8e8]'}`}>
      ...
      <section className="bg-white relative overflow-hidden" style={sectionStyle}>
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {cfg.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={cfg.heroTextTheme === 'dark' ? 'text-[#0f0f0f]' : 'text-white'} />}
          </div>
        ) : isSplitHero ? (
  ...
  const categoriesSection = landingCategories.length > 0 && (
    <section key="categories" className="border-t border-[#e8e8e8]" style={getSectionBackgroundStyle(tokens, 'categories')}>
        ...
        <div className={`grid ${LANDING_CATEGORY_GRID_CLASS[tokens.landingCategoryColumns]} gap-3`}>
          {landingCategories.map(category => (
            <Link
              ...
              className="flex flex-col items-center justify-center gap-2 border border-[#e8e8e8] bg-white hover:border-[#0f0f0f] transition-colors px-4 py-6 text-center uppercase text-xs font-bold tracking-wide text-[#0f0f0f]"
            >
    ...
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
        <div className="flex items-end justify-between mb-6">
          <h2 className="flex items-center gap-2 font-black text-2xl text-[#0f0f0f] uppercase tracking-tight">
          ...
          <Link href={`/products`} className="text-sm font-bold uppercase tracking-wide hover:underline" style={{ color: tokens.accentColor }}>
            {t.home.viewAll}
          </Link>
        </div>

        {products.length === 0 ? (
          <p className="text-[#8a8a8a] text-sm py-20 text-center">{t.home.noProductsYet}</p>
        ) : (
  ...
  const collectionsSection = landingCollections.length > 0 && (
    <section key="collections" className="border-t border-[#e8e8e8]" style={getSectionBackgroundStyle(tokens, 'collections')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14 flex flex-col gap-14">
        {landingCollections.map(collection => (
          <ProductScrollRow
            ...
            titleClassName="flex items-center gap-2 font-black text-2xl text-[#0f0f0f] uppercase tracking-tight"
            viewAllClassName="text-sm font-bold uppercase tracking-wide hover:underline"
            viewAllStyle={{ color: tokens.accentColor }}
            emptyMessageClassName="text-[#8a8a8a] text-sm py-16 text-center"
          />
        ))}
      </div>
    </section>
  ...
  const contentSection = hasContentBlock(tokens) && (
    <section key="content" className="border-t border-[#e8e8e8]" style={getSectionBackgroundStyle(tokens, 'content')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
        <ContentBlock
          tokens={tokens}
  ...
  const faqSection = tokens.showFaqSection && tokens.faqItems.length > 0 && (
    <section key="faq" className="border-t border-[#e8e8e8]" style={getSectionBackgroundStyle(tokens, 'faq')}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-14">
        <FaqAccordion
          tokens={tokens}
          t={t}
          headingClassName="font-black text-2xl sm:text-3xl text-[#0f0f0f] uppercase tracking-tight mb-6"
          questionClassName="font-bold text-sm text-[#0f0f0f]"
          answerClassName="text-sm text-[#6b6b6b] leading-relaxed"
          borderClassName="divide-[#e8e8e8]"
        />
      </div>
    </section>
  )
