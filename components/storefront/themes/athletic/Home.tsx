  ...
  function buildHero(t: Required<ThemeConfig>) {
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {t.heroEyebrow && (
          <p className={`flex items-center gap-2 ${HERO_EYEBROW_SIZE_CLASS[t.heroEyebrowSize]} font-black uppercase tracking-widest ${getHeroTextColorClass(t, 'eyebrow', 'text-[#6b6b6b]')} mb-3`}>
            <span className="inline-block w-1.5 h-1.5 shrink-0" style={{ backgroundColor: t.accentColor }} aria-hidden />
            {t.heroEyebrow}
          </p>
        )}
        <h1 className={`font-black uppercase tracking-tight mb-5 ${getHeroTextColorClass(t, 'headline', 'text-[#0f0f0f]')} ${HERO_HEADLINE_SIZE_CLASS[t.heroHeadlineSize]}`}>
          {t.heroHeadline || store.name}
        </h1>
        {t.heroSubheadline && (
          <p className={`${getHeroTextColorClass(t, 'subheadline', 'text-[#6b6b6b]')} ${HERO_SUBHEADLINE_SIZE_CLASS[t.heroSubheadlineSize]} mb-8`}>{t.heroSubheadline}</p>
        )}
      ...
      <section className="bg-white relative overflow-hidden" style={sectionStyle}>
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {t.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={t.heroTextTheme === 'dark' ? 'text-[#0f0f0f]' : 'text-white'} />}
          </div>
        ) : isSplitHero ? (
  ...
  const productsSection = (
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
        ...
        {products.length === 0 ? (
          <p className="text-[#8a8a8a] text-sm py-20 text-center">პროდუქტები ჯერ არ არის.</p>
        ) : (
