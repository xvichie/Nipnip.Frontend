  ...
  return (
    <div className="bg-white min-h-screen">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 pb-24">
        <div className="mb-8">
          <p className="text-xs font-semibold uppercase tracking-widest text-[#8a8a8a] mb-1">{t.grid.collectionEyebrow}</p>
          <h1 className="flex items-center gap-2 font-black text-3xl text-[#0f0f0f] tracking-tight">
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <aside className="w-full md:w-56 shrink-0 flex flex-col gap-4 md:sticky md:top-14 md:self-start md:max-h-[calc(100vh-4.5rem)] md:overflow-y-auto md:pb-6">
            {displayCategories.length > 0 && (
              <div className="border border-[#e8e8e8] bg-white p-4 flex flex-col gap-1">
                <p className="text-xs font-semibold uppercase tracking-wide text-[#8a8a8a] mb-2">{t.header.categories}</p>
                <Link
                  href={`/products`}
                  className={[
                    'px-3 py-2 text-sm transition-colors',
                    !activeCategorySlug ? 'bg-[#0f0f0f] text-white font-semibold' : 'text-[#6b6b6b] hover:bg-[#f7f7f7]',
                  ].join(' ')}
                >
                ...
                {displayCategories.map(category => (
                  <Link
                    ...
                    className={[
                      ...
                      activeCategorySlug === category.slug ? 'bg-[#0f0f0f] text-white font-semibold' : 'text-[#6b6b6b] hover:bg-[#f7f7f7]',
            ...
            {bounds && (
              <div className="border border-[#e8e8e8] bg-white p-4">
                <PriceRangeFilter
                  ...
                  t={t}
                  trackColorClassName="bg-[#e8e8e8]"
                  labelClassName="text-[#8a8a8a]"
            ...
            {(facets ?? []).length > 0 && (
              <div className="border border-[#e8e8e8] bg-white p-4 flex flex-col gap-4">
                <OptionFiltersPanel
                  facets={facets ?? []}
                  ...
                  accentColor={tokens.accentColor}
                  labelClassName="text-[#8a8a8a]"
                  chipClassName="border-[#e8e8e8] bg-white text-[#6b6b6b] hover:bg-[#f7f7f7]"
                />
              </div>
            )}
          </aside>
          ...
          <div className="flex-1">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
              <p className="text-sm text-[#6b6b6b]">
                <span className="text-[#0f0f0f] font-semibold">{totalCount}</span> {t.grid.unitProduct}
              </p>
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="w-full sm:w-56 bg-white border border-[#e8e8e8] text-sm px-3 py-2 text-[#0f0f0f] placeholder:text-[#8a8a8a] focus:outline-none focus:border-[#0f0f0f] transition-colors"
                />
                <select
                  value={sortBy}
                  onChange={e => setSortBy(e.target.value as ProductSortOption)}
                  className="bg-white border border-[#e8e8e8] text-sm px-3 py-2 text-[#0f0f0f] focus:outline-none focus:border-[#0f0f0f] transition-colors"
                >
            ...
            {isLoading ? (
              ...
              <div className="py-32 flex flex-col items-center gap-4 text-center">
                <p className="text-[#8a8a8a] text-sm">{t.grid.noProductsFound}</p>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                ...
                <Pagination
                  ...
                  onChange={setPage}
                  t={t}
                  buttonClassName="btn btn-sm bg-white border border-[#e8e8e8] text-[#6b6b6b] hover:text-[#0f0f0f] disabled:opacity-30"
                  textClassName="text-[#8a8a8a]"
                />
              </>
            )}
          </div>
