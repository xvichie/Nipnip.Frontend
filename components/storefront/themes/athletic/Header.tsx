...
export function Header({
  ...
  return (
    <header
      className={`${tokens.headerSticky ? 'sticky top-0' : ''} z-50 bg-white border-b border-[#e8e8e8]`}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-6">
          ...
          {tokens.logoUrl ? (
            <>
              ...
              {tokens.showStoreName && (
                <span className="font-black text-[#0f0f0f] text-lg uppercase tracking-tight truncate">{storeName}</span>
              )}
            </>
          ) : (
            <span className="font-black text-[#0f0f0f] text-lg uppercase tracking-tight truncate">{storeName}</span>
        ...
        {hasNav && (
          <nav className="hidden md:flex items-center gap-5">
            <Link href={`/products`} className="uppercase text-xs font-semibold tracking-wide text-[#6b6b6b] hover:text-[#0f0f0f] transition-colors whitespace-nowrap">
              ...
              isDropdown ? (
                ...
                >
                  <button
                    ...
                    className="flex items-center gap-1 uppercase text-xs font-semibold tracking-wide text-[#6b6b6b] hover:text-[#0f0f0f] transition-colors whitespace-nowrap"
                  >
                    {t.header.categories}
                  ...
                  {categoriesOpen && (
                    <div className="absolute top-full left-0 mt-1 min-w-[180px] bg-white shadow-lg border border-[#e8e8e8] py-1.5 flex flex-col z-10">
                      {navCategories.map(category => (
                        <Link
                          key={category.id}
                          href={`/products/category/${category.slug}`}
                          onClick={() => setCategoriesOpen(false)}
                          className={`py-2 text-sm text-[#6b6b6b] hover:text-[#0f0f0f] hover:bg-[#f7f7f7] transition-colors whitespace-nowrap ${category.isChild ? 'pl-8 pr-4' : 'px-4'}`}
                        >
                ...
                navCategories.map(category => (
                  <Link
                    key={category.id}
                    href={`/products/category/${category.slug}`}
                    className={`uppercase text-xs font-semibold tracking-wide text-[#6b6b6b] hover:text-[#0f0f0f] transition-colors whitespace-nowrap ${category.isChild ? 'pl-3' : ''}`}
                  >
            ...
            {navPages.map(page => (
              <Link
                ...
                className="uppercase text-xs font-semibold tracking-wide text-[#6b6b6b] hover:text-[#0f0f0f] transition-colors whitespace-nowrap"
              >
                {page.title}
              </Link>
            ))}
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          {!isCheckout && (
            <Link
              href={`/cart`}
              className="relative flex items-center gap-2 px-3 py-2 text-xs font-black uppercase tracking-wide text-white bg-[#0f0f0f] hover:bg-[#1a1a1a] transition-colors"
            >
          ...
          {hasNav && (
            <button
              ...
              className="md:hidden flex items-center justify-center w-9 h-9 text-[#6b6b6b] hover:text-[#0f0f0f] hover:bg-[#f2f2f2] transition-colors"
            >
        ...
        <div className="md:hidden fixed inset-0 z-50">
          ...
          <div className="absolute top-0 right-0 h-full w-72 max-w-[80vw] bg-white shadow-xl flex flex-col p-5 overflow-y-auto">
            <button
              ...
              className="self-end mb-3 w-8 h-8 flex items-center justify-center text-[#6b6b6b] hover:text-[#0f0f0f]"
            >
            ...
            <Link
              href={`/products`}
              onClick={() => setMobileOpen(false)}
              className="uppercase text-xs font-semibold tracking-wide text-[#0f0f0f] py-2.5 border-b border-[#efefef]"
            >
              {t.header.allProducts}
            </Link>
            {navCategories.map(category => (
              <Link
                ...
                href={`/products/category/${category.slug}`}
                onClick={() => setMobileOpen(false)}
                className={`uppercase text-xs font-semibold tracking-wide text-[#6b6b6b] py-2.5 border-b border-[#efefef] ${category.isChild ? 'pl-4' : ''}`}
              >
                {getCategoryName(category, lang)}
              </Link>
            ))}
            {navPages.map(page => (
              <Link
                ...
                href={`/pages/${page.slug}`}
                onClick={() => setMobileOpen(false)}
                className="uppercase text-xs font-semibold tracking-wide text-[#6b6b6b] py-2.5 border-b border-[#efefef]"
              >
                {page.title}
              </Link>
            ))}
