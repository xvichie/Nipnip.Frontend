...
export function Home({
  ...
  const heroText = (
    <div className={`w-full max-w-xl ${textAlignClass}`}>
      {tokens.heroEyebrow && (
        <p className="text-xs uppercase tracking-[0.3em] text-[#9c7a4a] mb-4">{tokens.heroEyebrow}</p>
      )}
      <h1 className={`font-serif text-[#1c1a17] tracking-tight mb-6 ${isSplitHero ? 'text-3xl sm:text-4xl' : 'text-4xl sm:text-6xl'}`}>
