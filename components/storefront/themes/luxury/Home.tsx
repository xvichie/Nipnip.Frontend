...
export function Home({
  ...
  function buildHero(cfg: Required<ThemeConfig>) {
    ...
    const sectionStyle = sectionBanner
      ? cfg.bannerType === 'image'
        ? {
            backgroundImage: `linear-gradient(rgba(250,247,242,0.8),rgba(250,247,242,0.92)), url(${cfg.bannerUrl})`,
    ...
    const heroText = (
      <div className={`w-full max-w-xl ${textAlignClass}`}>
        {heroEyebrow && (
          <p className={`${HERO_EYEBROW_SIZE_CLASS[cfg.heroEyebrowSize]} uppercase tracking-[0.3em] ${getHeroTextColorClass(cfg, 'eyebrow', 'text-[#9c7a4a]')} mb-4`}>{heroEyebrow}</p>
        )}
        <h1 className={`font-serif tracking-tight mb-6 ${getHeroTextColorClass(cfg, 'headline', 'text-[#1c1a17]')} ${HERO_HEADLINE_SIZE_CLASS[cfg.heroHeadlineSize]}`}>
          {heroHeadline || store.name}
        </h1>
        {heroSubheadline && (
          <p className={`${getHeroTextColorClass(cfg, 'subheadline', 'text-[#6b6255]')} ${HERO_SUBHEADLINE_SIZE_CLASS[cfg.heroSubheadlineSize]} mb-9`}>{heroSubheadline}</p>
        )}
      ...
      <section className="relative overflow-hidden" style={sectionStyle}>
        {isBackgroundHero ? (
          <div className={`relative flex flex-col ${pos.wrapper} ${heightClass}`}>
            ...
            {cfg.heroScrollIndicatorEnabled && <ScrollIndicator colorClassName={cfg.heroTextTheme === 'dark' ? 'text-[#1c1a17]' : 'text-white'} />}
          </div>
        ) : isSplitHero ? (
  ...
  const categoriesSection = landingCategories.length > 0 && (
    <section key="categories" className="border-t border-[#1c1a17]/10" style={getSectionBackgroundStyle(tokens, 'categories')}>
    ...
    <section key="products" style={getSectionBackgroundStyle(tokens, 'products')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
        <div className="flex items-end justify-between mb-8">
          <h2 className="font-serif text-2xl text-[#1c1a17]">{t.home.allProductsHeading}</h2>
        ...
        {products.length === 0 ? (
          <p className="text-[#9c8f7e] text-sm py-20 text-center">{t.home.noProductsYet}</p>
        ) : (
  ...
  const collectionsSection = landingCollections.length > 0 && (
    <section key="collections" className="border-t border-[#1c1a17]/10" style={getSectionBackgroundStyle(tokens, 'collections')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 flex flex-col gap-16">
        {landingCollections.map(collection => (
          <ProductScrollRow
            ...
            titleClassName="font-serif text-2xl text-[#1c1a17]"
            ...
            emptyMessageClassName="text-[#9c8f7e] text-sm py-16 text-center"
          />
        ))}
      </div>
    </section>
  ...
  const contentSection = hasContentBlock(tokens) && (
    <section key="content" className="border-t border-[#1c1a17]/10" style={getSectionBackgroundStyle(tokens, 'content')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
        <ContentBlock
          tokens={tokens}
          lang={lang}
  ...
  const faqSection = tokens.showFaqSection && tokens.faqItems.length > 0 && (
    <section key="faq" className="border-t border-[#1c1a17]/10" style={getSectionBackgroundStyle(tokens, 'faq')}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-16">
        <FaqAccordion
          tokens={tokens}
          t={t}
          lang={lang}
          headingClassName="font-serif text-2xl sm:text-3xl text-[#1c1a17] tracking-tight mb-6"
          questionClassName="text-sm text-[#1c1a17]"
          answerClassName="text-sm text-[#6b6255] leading-relaxed"
          borderClassName="divide-[#1c1a17]/10"
        />
      </div>
    </section>
  )
  ...
  return (
    <div className="bg-[#faf7f2] min-h-screen">
      {getHomeSectionOrder(tokens).map(key => {
        ...
        return (
          <section key={key} className="border-t border-[#1c1a17]/10" style={{ backgroundColor: section.backgroundColor || undefined }}>
            <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
              <CustomSectionBlock
                section={section}
                lang={lang}
