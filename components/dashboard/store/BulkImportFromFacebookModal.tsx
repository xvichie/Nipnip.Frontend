...
export function BulkImportFromFacebookModal() {
  ...
  const { data: posts, isLoading: postsLoading } = useFacebookPosts(open && connected)
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg rounded-2xl border border-white/10 bg-[#14141c] flex flex-col overflow-hidden max-h-[90vh]">
            <div className="flex items-center justify-between px-5 pt-5">
              <h3 className="text-sm font-semibold text-white">Bulk import from Facebook</h3>
              <button
                type="button"
                onClick={close}
                aria-label="Close"
                className="w-7 h-7 flex items-center justify-center text-white/40 hover:text-white disabled:opacity-30"
                disabled={importing}
              >
            ...
            <div className="flex flex-col gap-4 px-5 pt-4 pb-5 overflow-y-auto">
              {statusLoading ? (
                ...
                <>
                  <p className="text-white/40 text-xs leading-relaxed">
                    Every recent post is selected by default — untick anything you don&apos;t want imported, then
                    import the rest as products in one go. AI fills in name, price, sizes/colors, and category for
                    each; review and fix up anything it misses afterward.
                  </p>
