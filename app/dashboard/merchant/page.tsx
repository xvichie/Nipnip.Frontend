...
import { useMerchantDashboard } from '@/lib/queries/merchants'
...
export default function MerchantDashboardPage() {
  ...
  const { data, isLoading, isError } = useMerchantDashboard(fromISO, toISO)
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div>
        <h1 className="text-2xl font-black tracking-tight">{t.merchantDashboard.title}</h1>
        <p className="text-white/40 text-sm mt-1">{t.merchantDashboard.subtitle}</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          />
          <StatCard
            label={t.merchantDashboard.conversions}
      ...
      <div className="rounded-2xl border border-white/7 bg-white/2 overflow-hidden">
        <div className="px-6 py-4 border-b border-white/6">
          <h2 className="text-xs font-semibold text-white/40 uppercase tracking-widest">{t.merchantDashboard.topCreators}</h2>
        </div>
        ...
        ) : isError ? null : data!.topCreators.length === 0 ? (
          <div className="px-6 py-16 text-center flex flex-col items-center gap-2">
            <p className="text-white/20 text-sm">{t.merchantDashboard.noCreators}</p>
            <p className="text-white/15 text-xs">{t.merchantDashboard.noCreatorsHint}</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/6">
                  {[t.merchantDashboard.tableNum, t.merchantDashboard.tableCreator, t.merchantDashboard.tableClicks, t.merchantDashboard.tableConversions, t.merchantDashboard.tableCommission].map((h, i) => (
                    <th
                      key={h}
                      className={[
              ...
              <tbody>
                {data!.topCreators.map((c, i) => (
                  <tr key={c.creatorId} className="border-b border-white/4 last:border-0 hover:bg-white/2 transition-colors">
                    <td className="px-6 py-4 text-white/25 font-mono text-sm">{i + 1}</td>
                    <td className="px-6 py-4">
                      <span className="font-medium text-white text-sm">{c.creatorName}</span>
                      <span className="text-white/25 text-xs ml-2">@{c.creatorSlug}</span>
                    </td>
