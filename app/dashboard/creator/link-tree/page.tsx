...
export default function LinkTreePage() {
  ...
  const { data: linkTree, isLoading, isError } = useMyLinkTree()
  ...
  const updateItem = useUpdateLinkTreeItem()
  const deleteItem = useDeleteLinkTreeItem()
  const reorder = useReorderLinkTree()

  const items = linkTree?.items ?? []
  ...
  const bioUrl = creator && typeof window !== 'undefined' ? `${window.location.origin}/${creator.slug}` : ''
  ...
  async function handleAdd() {
    ...
    setAddError('')
    try {
      await addItem.mutateAsync({ merchantId: selectedMerchantId, label: newLabel.trim() || null })
      ...
      <div className="rounded-2xl border border-white/7 bg-white/2 p-5 flex flex-col gap-3">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          <select
            ...
            className="select flex-1 bg-white/4 border-white/8 focus:border-violet-500/60"
          >
